...
public class RequiredCreatorTest extends BaseMapTest
{
    ...
    public void testRequiredAnnotatedParam() throws Exception
    {
        ...
        p = POINT_READER.readValue(com.fasterxml.jackson.VPackUtils.toBytes(aposToQuotes("{'y':2,'x':1}")));
        assertEquals(1, p.x);
        assertEquals(2, p.y);
        p = POINT_READER.readValue(com.fasterxml.jackson.VPackUtils.toBytes(aposToQuotes("{'x':3,'y':4}")));
        assertEquals(3, p.x);
        assertEquals(4, p.y);

        // also fine if 'y' is MIA
        p = POINT_READER.readValue(com.fasterxml.jackson.VPackUtils.toBytes(aposToQuotes("{'x':3}")));
        assertEquals(3, p.x);
        assertEquals(0, p.y);

        // but not so good if 'x' missing
        try {
            POINT_READER.readValue(com.fasterxml.jackson.VPackUtils.toBytes(aposToQuotes("{'y':3}")));
    ...
    public void testRequiredGloballyParam() throws Exception
    {
        ...
        p = POINT_READER.readValue(com.fasterxml.jackson.VPackUtils.toBytes(aposToQuotes("{'x':2}")));
        assertEquals(2, p.x);
        assertEquals(0, p.y);
        ...
        try {
            r.readValue(com.fasterxml.jackson.VPackUtils.toBytes(aposToQuotes("{'x':6}")));
            fail("Should not pass");
        } catch (JsonMappingException e) {
