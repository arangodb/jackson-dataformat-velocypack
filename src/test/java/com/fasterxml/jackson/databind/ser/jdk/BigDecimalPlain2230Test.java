...
public class BigDecimalPlain2230Test extends BaseMapTest
{
    ...
    public void testBigIntegerAsPlainTest() throws Exception
    {
        ...
        assertEquals("{\"value\":\""+BD_VALUE.toString()+"\"}", com.fasterxml.jackson.VPackUtils.toJson( MAPPER.writeValueAsBytes(INPUT)));

        // but can force to "plain" notation
        final ObjectMapper m = jsonMapperBuilder()
            .enable(StreamWriteFeature.WRITE_BIGDECIMAL_AS_PLAIN)
            .build();
        assertEquals("{\"value\":\""+NORM_VALUE+"\"}", com.fasterxml.jackson.VPackUtils.toJson( m.writeValueAsBytes(INPUT)));
    }
}
