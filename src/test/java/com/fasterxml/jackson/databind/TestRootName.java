...
public class TestRootName extends BaseMapTest
{
    ...
    public void testRootViaWriterAndReader() throws Exception
    {
        ...
        assertEquals("{\"rudy\":{\"a\":3}}", json);
        Bean bean = mapper.readerFor(Bean.class).readValue(com.fasterxml.jackson.VPackUtils.toVPack(json));
        assertNotNull(bean);
    }
    ...
    public void testReconfiguringOfWrapping() throws Exception
    {
        ...
        try { // must not have extra wrapping
            result = mapper.readerFor(Bean.class).with(DeserializationFeature.UNWRAP_ROOT_VALUE)
                .readValue(com.fasterxml.jackson.VPackUtils.toVPack(jsonUnwrapped));
        ...
        result = mapper.readerFor(Bean.class).with(DeserializationFeature.UNWRAP_ROOT_VALUE)
            .readValue(com.fasterxml.jackson.VPackUtils.toVPack(jsonWrapped));
        assertNotNull(result);
    }
    ...
    public void testRootUsingExplicitConfig() throws Exception
    {
        ...
        Bean bean = reader.readValue(com.fasterxml.jackson.VPackUtils.toVPack(json));
        assertNotNull(bean);
        ...
        bean = wrapping.readerFor(Bean.class).withRootName("").readValue(com.fasterxml.jackson.VPackUtils.toVPack(json));
        assertNotNull(bean);
        assertEquals(3, bean.a);

        bean = wrapping.readerFor(Bean.class).withoutRootName().readValue(com.fasterxml.jackson.VPackUtils.toVPack("{\"a\":4}"));
        assertNotNull(bean);
        assertEquals(4, bean.a);

        // and back to defaults
        bean = wrapping.readerFor(Bean.class).readValue(com.fasterxml.jackson.VPackUtils.toVPack("{\"rudy\":{\"a\":7}}"));
        assertNotNull(bean);
