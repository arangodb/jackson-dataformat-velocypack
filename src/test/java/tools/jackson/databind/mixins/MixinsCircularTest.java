...
public class MixinsCircularTest extends DatabindTestUtil {
    ...
    public void testPojoMixinDeserialization() throws Exception {
        ObjectMapper mxMapper = vpackMapperBuilder()
        ...
        First first = mxMapper.readValue(VPackUtils.toVPack(a2q("{'second-mixin':'second-mixin'}")), First.class);
        assertEquals("second-mixin", first.value);

        // second deserialized from third
        Second second = mxMapper.readValue(VPackUtils.toVPack(a2q("{'third-mixin':'third-mixin'}")), Second.class);
        assertEquals("third-mixin", second.value);

        // third deserialized from first
        Third third = mxMapper.readValue(VPackUtils.toVPack(a2q("{'first-mixin':'first-mixin'}")), Third.class);
    ...
    public void testPojoMixinSerialization() throws Exception {
        ...
        assertEquals(
            ...
            VPackUtils.toJson(mxMapper.writeValueAsBytes(firstBean)));
        ...
        assertEquals(
            ...
            VPackUtils.toJson(mxMapper.writeValueAsBytes(secondBean)));
        ...
        assertEquals(
            ...
            VPackUtils.toJson(mxMapper.writeValueAsBytes(thirdBean)));
    }
}
