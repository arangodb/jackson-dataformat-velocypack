...
public class WrapperNameAsPropertyNameTest extends DatabindTestUtil
{
    ...
    private final ObjectMapper MAPPER_WITH_WRAPPER = vpackMapperBuilder()
            .enable(MapperFeature.USE_WRAPPER_NAME_AS_PROPERTY_NAME)
            .annotationIntrospector(new WrapperNameIntrospector())
            .build();

    private final ObjectMapper MAPPER_WITHOUT_WRAPPER = vpackMapperBuilder()
    ...
    {
        String json = VPackUtils.toJson(MAPPER_WITH_WRAPPER.writeValueAsBytes(new WrapperBean(42, "Bob")));
    ...
    public void testDeserializationWithWrapperName() throws Exception
    {
        WrapperBean bean = MAPPER_WITH_WRAPPER.readValue(
                VPackUtils.toVPack(a2q("{'wrappedValue':42,'wrappedName':'Bob'}")),
                WrapperBean.class);
    ...
    public void testSerializationWithoutFeature() throws Exception
    {
        ...
        String json = VPackUtils.toJson(MAPPER_WITHOUT_WRAPPER.writeValueAsBytes(new WrapperBean(42, "Bob")));
    ...
    public void testDeserializationWithoutFeature() throws Exception
    {
        ...
        WrapperBean bean = MAPPER_WITHOUT_WRAPPER.readValue(
                VPackUtils.toVPack(a2q("{'value':42,'name':'Bob'}")),
                WrapperBean.class);
        assertEquals(42, bean.value);
        assertEquals("Bob", bean.name);
    }
