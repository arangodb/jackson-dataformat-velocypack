...
public class RecordCreatorsTest extends DatabindTestUtil
{
    ...
    private final ObjectMapper MAPPER = vpackMapperBuilder().enable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES).build();
    ...
    public void testDeserializeWithCanonicalCtorOverride() throws Exception {
        RecordWithCanonicalCtorOverride value = MAPPER.readValue(VPackUtils.toVPack("{\"id\":123,\"name\":\"Bob\"}"),
    ...
    public void testDeserializeWithAltCtor() throws Exception {
        RecordWithAltCtor value = MAPPER.readValue(VPackUtils.toVPack("{\"id\":2812}"),
        ...
        try {
            MAPPER.readValue(VPackUtils.toVPack("{\"id\":2812,\"name\":\"Bob\"}"),
                    RecordWithAltCtor.class);
    ...
    public void testDeserializeWithDelegatingCtor() throws Exception {
        RecordWithDelegation value = MAPPER.readValue(VPackUtils.toVPack(q("foobar")),
                RecordWithDelegation.class);
        assertEquals("del:foobar", value.accessValueForTest());

        assertEquals(q("val:del:foobar"), VPackUtils.toJson(MAPPER.writeValueAsBytes(value)));
    }
    ...
    {
        String result = VPackUtils.toJson(MAPPER.writeValueAsBytes(new PlainTestObject("test", 1)));
    ...
    {
        String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(new CreatorTestObject("test", 2, 1)));

        @SuppressWarnings("unchecked")
        Map<String, Object> asMap = (Map<String, Object>) MAPPER.readValue(VPackUtils.toVPack(json), Map.class);
    ...
    void testDeserializeWithCreatorAndJsonValue4724() throws Exception {
        newVPackMapper().readValue(VPackUtils.toVPack("\"\""), Something.class);
    }
    ...
    public void testFailingSetter3938() throws Exception
    {
        ...
        assertNotNull(R.readValue(VPackUtils.toVPack(a2q("{'id':1, 'filter':'abc'}"))));
        assertNotNull(R.readValue(VPackUtils.toVPack(a2q("{'id':2, 'filter':'abc', 'options':null}"))));
    ...
    public void testRecordWithAnySetterCtor() throws Exception
    {
        ...
        RecordWithAnySetterCtor562 result = mapper.readValue(VPackUtils.toVPack(a2q("{'regular':13}")),
        ...
        result = mapper.readValue(VPackUtils.toVPack(a2q("{'regular':13, 'unknown':99, 'extra':-1}")),
                RecordWithAnySetterCtor562.class);
        assertEquals(13, result.id);
    ...
    public void testJsonAnySetterOnRecord() throws Exception {
        ...
        TestRecord3439 result = mapper.readValue(VPackUtils.toVPack(json), TestRecord3439.class);
    ...
    public void testDeserialization5923True() throws Exception {
        Outer5923 bw = MAPPER.readValue(VPackUtils.toVPack("{ \"renamed\": true }"), Outer5923.class);
    ...
    public void testDeserialization5923False() throws Exception {
        Outer5923 bw = MAPPER.readValue(VPackUtils.toVPack("{ \"renamed\": false }"), Outer5923.class);
    ...
    public void testSerialization5923ViaJsonValue() throws Exception {
        assertEquals("{\"renamed\":true}",
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new Outer5923(new Inner5923(true)))));
        assertEquals("{\"renamed\":false}",
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new Outer5923(new Inner5923(false)))));
    }
    ...
    public void testRoundTrip5923() throws Exception {
        ...
        Outer5923 roundTripped = MAPPER.readValue(VPackUtils.toVPack(VPackUtils.toJson(MAPPER.writeValueAsBytes(original))), Outer5923.class);
