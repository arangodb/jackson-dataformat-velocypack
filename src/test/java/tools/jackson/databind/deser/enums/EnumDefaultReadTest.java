...
public class EnumDefaultReadTest
{
    ...
    public void testEmptyStringAsDefault() throws Exception
    {
        ...
        assertEquals(SimpleEnumWithDefault.ONE,
                r.readValue(VPackUtils.toVPack(q("ONE"))));
        assertEquals(SimpleEnumWithDefault.ZERO,
                r.readValue(VPackUtils.toVPack(q("Zero"))));
        assertEquals(SimpleEnumWithDefault.ZERO,
                r.readValue(VPackUtils.toVPack(q(""))));
        assertEquals(SimpleEnumWithDefault.ZERO,
                r.readValue(VPackUtils.toVPack(q("    "))));
        ...
        assertNull(r2.readValue(VPackUtils.toVPack(q(""))));
        assertNull(r2.readValue(VPackUtils.toVPack(q("    "))));
    }
    ...
    private <T> void _verifyOkDeserialization(ObjectReader reader, String fromValue,
        ...
        throws Exception
    {
        assertEquals(expValue, reader.forType(toValueType).readValue(VPackUtils.toVPack(q(fromValue))));
    }
        ...
        throws Exception
    {
        InvalidFormatException e = assertThrows(InvalidFormatException.class,
                () -> reader.forType(toValueType).readValue(VPackUtils.toVPack(q(fromValue))));
    ...
    public void testEnumDefaultValueViaMixin() throws Exception 
    {
        ...
        assertEquals(BaseEnumDefault.Z, 
                mixinMapper.readValue(VPackUtils.toVPack(q("UNKNOWN")), BaseEnumDefault.class));
    }
    ...
    public void testFirstEnumDefaultValueViaMixin() throws Exception
    {
        ...
        assertEquals(BaseOverloaded.A,
                mixinMapper.readValue(VPackUtils.toVPack(q("UNKNOWN")), BaseOverloaded.class));
    }
    ...
    {
        ObjectReader r = newVPackMapper().readerFor(Brand4403.class)
                .with(EnumFeature.READ_UNKNOWN_ENUM_VALUES_USING_DEFAULT_VALUE);
        assertEquals(Brand4403.SEAT, r.readValue(VPackUtils.toVPack(q("005"))));
        assertEquals(Brand4403.HYUNDAI, r.readValue(VPackUtils.toVPack(q("006"))));
        assertEquals(Brand4403.OTHER, r.readValue(VPackUtils.toVPack(q("x"))));

        // Problem here: "001" taken as "Stringified" index 1
        assertEquals(Brand4403.OTHER, r.readValue(VPackUtils.toVPack(q("001"))));
    }
}
