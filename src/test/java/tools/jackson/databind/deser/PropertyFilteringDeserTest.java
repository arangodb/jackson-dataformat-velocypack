...
public class PropertyFilteringDeserTest
{
    ...
    {
        SizeClassIgnore result = MAPPER.readValue(VPackUtils.toVPack("{ \"x\":1, \"y\" : 2 }"),
    ...
    public void testFailOnIgnore() throws Exception
    {
        ...
        // First, fine to get "x":
        NoYOrZ result = r.readValue(VPackUtils.toVPack(a2q("{'x':3}")));
        ...
        try {
            result = r.readValue(VPackUtils.toVPack(a2q("{'x':3, 'y':4}")));
            fail("Should fail");
        ...
        // or 'z'
        try {
            result = r.readValue(VPackUtils.toVPack(a2q("{'z':2 }")));
            fail("Should fail");
        } catch (IgnoredPropertyException e) {
            verifyException(e, "Ignored field");
        }
    ...
    public void testSimpleInclude() throws Exception
    {
        OnlyYAndZ result = MAPPER.readValue(
                VPackUtils.toVPack(a2q("{ 'x':1, '_x': 1, 'y':2, 'z':3 }")),
                OnlyYAndZ.class);
    ...
    public void testIncludeIgnoredAndUnrecognizedField() throws Exception
    {
        ...
        OnlyY result = r.readValue(VPackUtils.toVPack(a2q("{'x':3, 'y': 4}")));
        ...
        r = r.with(DeserializationFeature.FAIL_ON_IGNORED_PROPERTIES);
        try {
            r.readValue(VPackUtils.toVPack(a2q("{'x':3, 'y': 4, 'z': 5}")));
            fail("Should fail");
        } catch (IgnoredPropertyException e) {
            verifyException(e, "Ignored field");
        }
        ...
        try {
            r.readValue(VPackUtils.toVPack(a2q("{'y': 3, 'z':2 }")));
            fail("Should fail");
    ...
    public void testMergeInclude() throws Exception
    {
        OnlyYWrapperForOnlyYAndZ onlyY = MAPPER.readValue(
                VPackUtils.toVPack(a2q("{'onlyY': {'x': 2, 'y':3, 'z': 4}}")),
    ...
    public void testListInclude() throws Exception
    {
        IncludeForListValuesY result = MAPPER.readValue(
                VPackUtils.toVPack(a2q("{'onlyYs':[{ 'x':1, 'y' : 2, 'z': 3 }]}")),
    ...
    {
        MapWrapper result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'value': {'a': 2, 'b': 3}}")), MapWrapper.class);
    ...
    {
        MyMap result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'a': 2, 'b': 3}")), MyMap.class);
