...
public class GenericTypeDeserTest
{
    ...
    {
        NumberBean result = MAPPER.readValue(VPackUtils.toVPack("{\"number\":17}"), NumberBean.class);
    ...
    public void testMultipleWrappers() throws Exception
    {
        ...
        Wrapper<Boolean> result = MAPPER.readValue
            (VPackUtils.toVPack("{\"value\": true}"), new TypeReference<Wrapper<Boolean>>() { });
        assertEquals(new Wrapper<Boolean>(Boolean.TRUE), result);

        // Then string one
        Wrapper<String> result2 = MAPPER.readValue
            (VPackUtils.toVPack("{\"value\": \"abc\"}"), new TypeReference<Wrapper<String>>() { });
        assertEquals(new Wrapper<String>("abc"), result2);

        // And then number
        Wrapper<Long> result3 = MAPPER.readValue
            (VPackUtils.toVPack("{\"value\": 7}"), new TypeReference<Wrapper<Long>>() { });
    ...
    void recursiveWildcard4118() throws Exception {
        Tree<?> tree = MAPPER.readValue(VPackUtils.toVPack("[[[]]]"), new TypeReference<Tree<?>>() {
        });
    ...
    void deserWildcard4118() throws Exception {
        ...
        String serialized = VPackUtils.toJson(MAPPER.writeValueAsBytes(test));

        // When
        TestObject4118 deserialized = MAPPER.readValue(VPackUtils.toVPack(serialized), TestObject4118.class);
    ...
    void multiParamWithWildcard() throws Exception {
        ...
        PairContainer result = MAPPER.readValue(VPackUtils.toVPack(json), PairContainer.class);
    ...
    void multiParamWithBothWildcards() throws Exception {
        ...
        Pair<?, ?> result = MAPPER.readValue(VPackUtils.toVPack(json), Pair.class);
