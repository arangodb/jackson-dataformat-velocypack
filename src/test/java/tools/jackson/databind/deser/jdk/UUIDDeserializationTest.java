...
public class UUIDDeserializationTest
{
    ...
    public void testUUID() throws Exception
    {
        ...
        for (String value : new String[] {
            ...
            assertEquals(uuid,
                    r.without(DeserializationFeature.UNWRAP_SINGLE_VALUE_ARRAYS)
                        .readValue(VPackUtils.toVPack(q(value))));
        }
        ...
        for (int i = 0; i < chars.length(); ++i) {
            ...
            assertEquals(UUID.fromString(value).toString(),
                    r.readValue(VPackUtils.toVPack(q(value))).toString());
        }
        ...
        assertEquals(UUID.fromString(NULL_UUID),
                r.readValue(VPackUtils.toVPack(q(base64))));
    }
    ...
    public void testUUIDInvalid() throws Exception
    {
        ...
        try {
            MAPPER.readValue(VPackUtils.toVPack(q("abcde")), UUID.class);
            fail("Should fail on invalid UUID string");
        } catch (InvalidFormatException e) {
            verifyException(e, "UUID has to be represented by standard");
        }
        try {
            MAPPER.readValue(VPackUtils.toVPack(q("76e6d183-5f68-4afa-b94a-922c1fdb83fx")), UUID.class);
            fail("Should fail on invalid UUID string");
        } catch (InvalidFormatException e) {
        ...
        try {
            MAPPER.readValue(VPackUtils.toVPack(q("76e6d183-5f68-4afa-b94a-922c1fdb83f\u007F")), UUID.class);
            fail("Should fail on invalid UUID string");
        } catch (InvalidFormatException e) {
