...
public class CustomMapKeyDeserializationTest
{
    ...
    public void testCustomSerializer() throws Exception
    {
        assertEquals(a2q("{'id=a':'b'}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(Collections.singletonMap(new Key2454("a", true), "b"))));
    }

    @Test
    public void testCustomDeserializer() throws Exception
    {
        Map<Key2454, String> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'a':'b'}")),
    ...
    public void testAnnotationKeyDeserWithoutMapperOverride4444() throws Exception {
        ...
        Map<Key4444, String> result = MAPPER.readValue(VPackUtils.toVPack("{\"foo\":null}"), typeRef);
    ...
    public void testAnnotationKeyDeserWithMapperOverride4444() throws Exception {
        ...
        ObjectMapper mapper = vpackMapperBuilder().addModule(sm).build();
        TypeReference<Map<Key4444, String>> typeRef = new TypeReference<Map<Key4444, String>>() {};
        Map<Key4444, String> result = mapper.readValue(VPackUtils.toVPack("{\"foo\":null}"), typeRef);
    ...
    public void testCustomKeyDeserializerNested4680() throws Exception
    {
        ...
        ObjectMapper mapper = VPackMapper.builder().addModule(keySanitizationModule).build();

        Map<String, Object> result = mapper.readValue(VPackUtils.toVPack(json), new TypeReference<Map<String, Object>>() {});
