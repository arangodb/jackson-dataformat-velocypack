...
public class MapKeyDeserializationTest
{
    ...
    public void testBooleanMapKeyDeserialization() throws Exception
    {
        ...
        MapWrapper<?,?> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'map':{'true':'foobar'}}")), type);
        assertEquals(1, result.map.size());
        assertEquals(Boolean.TRUE, result.map.entrySet().iterator().next().getKey());

        result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'map':{'false':'foobar'}}")), type);
        assertEquals(1, result.map.size());
    ...
    public void testByteMapKeyDeserialization() throws Exception
    {
        ...
        MapWrapper<?,?> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'map':{'13':'foobar'}}")), type);
        assertEquals(1, result.map.size());
    ...
    public void testIntegerMapKeyDeserialization() throws Exception
    {
        ...
        MapWrapper<?,?> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'map':{'-3':'foobar'}}")), type);
        assertEquals(1, result.map.size());
    ...
    public void testLongMapKeyDeserialization() throws Exception
    {
        ...
        MapWrapper<?,?> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'map':{'42':'foobar'}}")), type);
        assertEquals(1, result.map.size());
    ...
    public void testFloatMapKeyDeserialization() throws Exception
    {
        ...
        MapWrapper<?,?> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'map':{'3.5':'foobar'}}")), type);
        assertEquals(1, result.map.size());
    ...
    public void testDoubleMapKeyDeserialization() throws Exception
    {
        ...
        MapWrapper<?,?> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'map':{'0.25':'foobar'}}")), type);
        assertEquals(1, result.map.size());
    ...
    public void testDeserializeKeyViaFactory() throws Exception
    {
        Map<FullName, Double> map =
            MAPPER.readValue(VPackUtils.toVPack("{\"first.last\": 42}"),
    ...
    public void testByteArrayMapKeyDeserialization() throws Exception
    {
        ...
        MapWrapper<byte[], String> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'map':{'"+encoded+"':'foobar'}}")),
    ...
    public void testEnumWithCreatorMapKeyDeserialization() throws Exception
    {
        ...
        final Map<TestEnum2725, String> output = MAPPER.readValue(VPackUtils.toVPack(json),
    ...
    {
        try {
            MAPPER.readValue(VPackUtils.toVPack("{ \"\": 0 }"), MAP_TYPE_2158);
            fail("Should no pass");
    ...
    public void testNormalizeKey() throws Exception
    {
        assertEquals(Collections.singletonMap(DummyDto2158.fromValue("foo"), 0),
                MAPPER.readValue(VPackUtils.toVPack("{ \"FOO\": 0 }"), MAP_TYPE_2158));
    }
    ...
    public void testKeyWithCtorAndCreator3143() throws Exception
    {
        ...
        Map<Key3143Ctor,Integer> map = MAPPER.readValue(VPackUtils.toVPack("{\"bar\":3}"),
    ...
    public void testKeyWith2Creators3143() throws Exception
    {
        ...
        Map<Key3143Factories,Integer> map = MAPPER.readValue(VPackUtils.toVPack("{\"Foo\":3}"),
    ...
    {
        try {
            MAPPER.readValue(VPackUtils.toVPack("{\"Foo\":3}"),
