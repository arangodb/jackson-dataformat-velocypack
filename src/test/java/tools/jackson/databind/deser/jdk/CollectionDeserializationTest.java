...
public class CollectionDeserializationTest
{
    ...
    public void testUntypedList() throws Exception
    {
        ...
        Object value = MAPPER.readValue(VPackUtils.toVPack(JSON), Object.class);
        assertNotNull(value);
    ...
    public void testExactStringCollection() throws Exception
    {
        ...
        String JSON = "[ \"a\", \"b\" ]";
        List<String> result = MAPPER.readValue(VPackUtils.toVPack(JSON), new TypeReference<ArrayList<String>>() { });
    ...
    public void testHashSet() throws Exception
    {
        ...
        EnumSet<Key> result = MAPPER.readValue(VPackUtils.toVPack(JSON), new TypeReference<EnumSet<Key>>() { });
        assertNotNull(result);
    ...
    {
        CustomList result = MAPPER.readValue(VPackUtils.toVPack(q("abc")), CustomList.class);
    ...
    public void testImplicitArrays() throws Exception
    {
        ...
        List<Integer> ints = mapper.readValue(VPackUtils.toVPack("4"), List.class);
        assertEquals(1, ints.size());
        assertEquals(Integer.valueOf(4), ints.get(0));
        List<String> strings = mapper.readValue(VPackUtils.toVPack(q("abc")), new TypeReference<ArrayList<String>>() { });
        assertEquals(1, strings.size());
        assertEquals("abc", strings.get(0));
        // and arrays:
        int[] intArray = mapper.readValue(VPackUtils.toVPack("-7"), int[].class);
        assertEquals(1, intArray.length);
        assertEquals(-7, intArray[0]);
        String[] stringArray = mapper.readValue(VPackUtils.toVPack(q("xyz")), String[].class);
        assertEquals(1, stringArray.length);
        assertEquals("xyz", stringArray[0]);

        // and then with Beans:
        List<XBean> xbeanList = mapper.readValue(VPackUtils.toVPack("{\"x\":4}"), new TypeReference<List<XBean>>() { });
        assertEquals(1, xbeanList.size());
        assertEquals(XBean.class, xbeanList.get(0).getClass());

        Object ob = mapper.readValue(VPackUtils.toVPack("{\"x\":29}"), XBean[].class);
    ...
    public void testFromEmptyString() throws Exception
    {
        ...
        List<?> result = r.forType(List.class).readValue(VPackUtils.toVPack(q("")));
        assertNull(result);
    }
    ...
    public void testArrayBlockingQueue() throws Exception
    {
        ...
        ArrayBlockingQueue<?> q = MAPPER.readValue(VPackUtils.toVPack("[1, 2, 3]"), ArrayBlockingQueue.class);
        assertNotNull(q);
    ...
    public void testIterableWithStrings() throws Exception
    {
        ...
        ListAsIterable w = MAPPER.readValue(VPackUtils.toVPack(JSON), ListAsIterable.class);
        assertNotNull(w);
    ...
    public void testIterableWithBeans() throws Exception
    {
        ...
        ListAsIterableX w = MAPPER.readValue(VPackUtils.toVPack(JSON), ListAsIterableX.class);
        assertNotNull(w);
    ...
    {
        try {
            MAPPER.readValue(VPackUtils.toVPack("[ \"KEY2\", false ]"), Key[].class);
    ...
    {
        try {
            MAPPER.readValue(VPackUtils.toVPack("[ \"xyz\", { } ]"), String[].class);
            fail("Should not pass");
        } catch (MismatchedInputException e) {
            verifyException(e, "Cannot deserialize value of type");
    ...
    {
        try {
            MAPPER.readValue(VPackUtils.toVPack("{\"keys\":[ \"KEY2\", false ]}"), KeyListBean.class);
            fail("Should not pass");
        } catch (MismatchedInputException e) {
            verifyException(e, "Cannot deserialize value of type");
            verifyException(e, "from Boolean value");
    ...
    public void testWrapExceptions() throws Exception
    {
        ...
        try {
            wrappingReader.readValue(VPackUtils.toVPack("[{}]"));
        ...
        try {
            noWrapReader.readValue(VPackUtils.toVPack("[{}]"));
    ...
    public void testAbstractListAndSet() throws Exception
    {
        ...
        ListAsAbstract list = MAPPER.readValue(VPackUtils.toVPack(JSON), ListAsAbstract.class);
        assertEquals(2, list.values.size());
        assertEquals(ArrayList.class, list.values.getClass());

        SetAsAbstract set = MAPPER.readValue(VPackUtils.toVPack(JSON), SetAsAbstract.class);
    ...
    public void testWrapExceptions3068() throws Exception
    {
        ...
        final ObjectMapper mapper = vpackMapperBuilder()
                .addModule(module)
                .build();
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new MyJobModel()));
    ...
    public void testSingletonCollections() throws Exception
    {
        ...
        byte[] vpack = MAPPER.writeValueAsBytes(Collections.singleton(new XBean(3)));
        Collection<XBean> result = MAPPER.readValue(vpack, xbeanListType);
        assertNotNull(result);
        assertEquals(1, result.size());
        assertEquals(3, result.iterator().next().x);

        vpack = MAPPER.writeValueAsBytes(Collections.singletonList(new XBean(28)));
        result = MAPPER.readValue(vpack, xbeanListType);
        assertNotNull(result);
        assertEquals(1, result.size());
    ...
    public void testUnmodifiableSet() throws Exception
    {
        ...
        String json = VPackUtils.toJson(mapper.writeValueAsBytes(theSet));

        assertEquals("[\"java.util.Collections$UnmodifiableSet\",[\"a\"]]", json);

        Set<?> result = mapper.readValue(VPackUtils.toVPack(json), Set.class);
        assertNotNull(result);
        assertEquals(1, result.size());
    }
    ...
    {
        try {
            MAPPER.readValue(VPackUtils.toVPack("[ \"acb\", null, 123 ]"), TreeSet.class);
            fail("Should not pass");
        } catch (MismatchedInputException e) {
    ...
    {
        Deque<?> dq = MAPPER.readValue(VPackUtils.toVPack("[1]"), Deque.class);
        assertNotNull(dq);
        assertEquals(1, dq.size());
        assertInstanceOf(Deque.class, dq);

        NavigableSet<?> ns = MAPPER.readValue(VPackUtils.toVPack("[ true ]"), NavigableSet.class);
    ...
    public void testCustomNumberCollectionDeserialize5522() throws Exception {
        ...
        CustomClassForNumber5522 result = mapper.readValue(VPackUtils.toVPack(jsonValue), CustomClassForNumber5522.class);
    ...
    public void testCustomStringCollectionDeserialize5522() throws Exception {
        ...
        CustomClassForString5522 result = mapper.readValue(VPackUtils.toVPack(jsonValue), CustomClassForString5522.class);
    ...
    public void testStringCollectionDeserializeInField5522() throws Exception {
        ...
        CustomClassForListField5522 result = mapper.readValue(VPackUtils.toVPack(jsonValue), CustomClassForListField5522.class);
