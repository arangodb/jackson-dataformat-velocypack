...
public class Base64DecodingTest
{
    ...
    {
        byte[] b = MAPPER.readValue(VPackUtils.toVPack(q(BASE64_HELLO)), byte[].class);
    ...
    private void _testInvalidBase64(ObjectMapper mapper, String value) throws Exception
    {
        ...
        try {
            MAPPER.readValue(VPackUtils.toVPack(q(value)), byte[].class);
...
//            verifyException(e, "Failed to decode");
        ...
        // and then tree model
        JsonNode tree = mapper.readTree(VPackUtils.toVPack(String.format("{\"foo\":\"%s\"}", value)));
