...
//        mapper.disable(MapperFeature.USE_GETTERS_AS_SETTERS);
        SecurityGroup sg = MAPPER.readValue(VPackUtils.toVPack(data), SecurityGroup.class);
    ...
    private final ObjectMapper GETTER_AS_SETTER_MAPPER = vpackMapperBuilder()
    ...
    {
        assertEquals("{\"list\":[]}",
                VPackUtils.toJson(GETTER_AS_SETTER_MAPPER.writeValueAsBytes(new RenamedToSameOnGetter())));
        String payload = "{\"list\":[1,2,3,4]}";
        RenamedToSameOnGetter foo = GETTER_AS_SETTER_MAPPER.readValue(VPackUtils.toVPack(payload), RenamedToSameOnGetter.class);
    ...
    public void testRenamedToDifferentOnGetter2283() throws Exception
    {
        assertEquals("{\"renamedList\":[]}",
                VPackUtils.toJson(GETTER_AS_SETTER_MAPPER.writeValueAsBytes(new RenamedToDifferentOnGetter())));
        String payload = "{\"renamedList\":[1,2,3,4]}";
        RenamedToDifferentOnGetter foo = GETTER_AS_SETTER_MAPPER.readValue(VPackUtils.toVPack(payload), RenamedToDifferentOnGetter.class);
        assertTrue(foo.getList().isEmpty(), "List should be empty");
    }
    ...
    {
        assertEquals("{\"renamedList\":[]}",
                VPackUtils.toJson(GETTER_AS_SETTER_MAPPER.writeValueAsBytes(new RenamedOnClass())));
        String payload = "{\"renamedList\":[1,2,3,4]}";
        RenamedOnClass foo = GETTER_AS_SETTER_MAPPER.readValue(VPackUtils.toVPack(payload), RenamedOnClass.class);
        assertTrue(foo.getList().isEmpty(), "List should be empty");
    }
}
