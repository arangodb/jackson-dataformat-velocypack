...
class NullConversionsAsEmptyPOJO2572Test extends DatabindTestUtil {
    ...
    void roundTripWithNonNullValues() throws Exception {
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new Outer(new Inner("inner"))));
        Outer result = mapper.readValue(VPackUtils.toVPack(json), Outer.class);
    ...
    void nullPOJOBecomesEmptyInstance() throws Exception {
        ObjectMapper mapper = vpackMapperBuilder()
                .changeDefaultNullHandling(h -> h.withOverrides(JsonSetter.Value.construct(Nulls.AS_EMPTY, Nulls.AS_EMPTY)))
                .build();
        Outer result = mapper.readValue(VPackUtils.toVPack(a2q("{'inner':null}")), Outer.class);
        assertNotNull(result);
    ...
    void asEmptyFailsWithNoUsableCreator() throws Exception {
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        try {
            mapper.readValue(VPackUtils.toVPack(a2q("{'inner':null}")), OuterWithNonPublicInner.class);
