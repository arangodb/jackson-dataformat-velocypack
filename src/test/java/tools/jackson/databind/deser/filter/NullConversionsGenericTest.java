...
//        @JsonSetter(nulls=JsonSetter.Nulls.AS_EMPTY)
    ...
    {
        GeneralEmpty<Point> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'value':null}")),
        ...
        try {
            /* NoCtorWrapper nogo =*/ MAPPER.readValue(VPackUtils.toVPack(a2q("{'value':null}")),
                    NoCtorWrapper.class);
    ...
    public void testEmptyStringToNullToEmptyPojo() throws Exception
    {
        GeneralEmpty<Point> result = MAPPER.readerFor(new TypeReference<GeneralEmpty<Point>>() { })
                ...
                .readValue(VPackUtils.toVPack(a2q("{'value':''}")));
        assertNotNull(result.value);
        Point p = result.value;
        assertEquals(0, p.x);
        assertEquals(0, p.y);
    ...
    {
        GeneralEmpty<List<String>> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'value':null}")),
                new TypeReference<GeneralEmpty<List<String>>>() { });
        assertNotNull(result.value);
        assertEquals(0, result.value.size());

        // but also non-String type, since impls vary
        GeneralEmpty<List<Integer>> result2 = MAPPER.readValue(VPackUtils.toVPack(a2q("{'value':null}")),
    ...
    {
        GeneralEmpty<Map<String,String>> result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'value':null}")),
