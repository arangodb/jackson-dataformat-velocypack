...
public class DeepVPackNodeSerTest
{
    private final ObjectMapper NO_LIMITS_MAPPER;
    {
        VPackFactory jsonFactory = VPackFactory.builder()
                ...
                .build();
        NO_LIMITS_MAPPER = VPackMapper.builder(jsonFactory).build();
    }
    ...
    {
        JsonNode jsonNode = NO_LIMITS_MAPPER.readTree(VPackUtils.toVPack(_nestedDoc(TEST_NESTING)));
        String json = VPackUtils.toJson(NO_LIMITS_MAPPER.writeValueAsBytes(jsonNode));
        assertNotNull(json);
    }
