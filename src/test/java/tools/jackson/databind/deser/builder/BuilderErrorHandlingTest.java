...
public class BuilderErrorHandlingTest extends DatabindTestUtil
{
    ...
    private final ObjectMapper MAPPER_WITH_WRAPPING = vpackMapperBuilder()
            .enable(DeserializationFeature.WRAP_EXCEPTIONS)
            .build();

    private final ObjectMapper MAPPER_NO_WRAPPING = vpackMapperBuilder()
    ...
    {
        try {
            MAPPER.readValue(VPackUtils.toVPack("123"), ValueClassXY.class);
            fail("Should not pass");
        } catch (MismatchedInputException e) {
    ...
    {
        ValidatingValue result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'a':'1','b':'2'}")), ValidatingValue.class);
    ...
    {
        try {
            MAPPER_WITH_WRAPPING.readValue(VPackUtils.toVPack(a2q("{'a':'1'}")), ValidatingValue.class);
    ...
    {
        try {
            JsonNode tree = MAPPER_WITH_WRAPPING.readTree(VPackUtils.toVPack(a2q("{'a':'1'}")));
    ...
    public void testFailingValidatingBuilderWithoutExceptionWrapping() throws Exception
    {
        try {
            MAPPER_NO_WRAPPING
                    .readValue(VPackUtils.toVPack(a2q("{'a':'1'}")), ValidatingValue.class);
            fail("Expected an exception");
    ...
    {
        try {
            JsonNode tree = MAPPER_NO_WRAPPING.readTree(VPackUtils.toVPack(a2q("{'a':'1'}")));
