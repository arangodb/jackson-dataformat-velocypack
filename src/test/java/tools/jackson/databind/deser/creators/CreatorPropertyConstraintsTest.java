...
public class CreatorPropertyConstraintsTest
{
    ...
    public void testRequiredAnnotatedParam() throws Exception
    {
        ...
        p = POINT_READER.readValue(VPackUtils.toVPack(a2q("{'y':2,'x':1}")));
        assertEquals(1, p.x);
        assertEquals(2, p.y);
        p = POINT_READER.readValue(VPackUtils.toVPack(a2q("{'x':3,'y':4}")));
        assertEquals(3, p.x);
        assertEquals(4, p.y);

        // also fine if 'y' is MIA
        p = POINT_READER.readValue(VPackUtils.toVPack(a2q("{'x':3}")));
        assertEquals(3, p.x);
        assertNull(p.y);

        // but not so good if 'x' missing
        try {
            POINT_READER.readValue(VPackUtils.toVPack(a2q("{'y':3}")));
    ...
    public void testRequiredGloballyParam() throws Exception
    {
        ...
        p = POINT_READER.readValue(VPackUtils.toVPack(a2q("{'x':2}")));
        assertEquals(2, p.x);
        assertNull(p.y);
        ...
        try {
            r.readValue(VPackUtils.toVPack(a2q("{'x':6}")));
            fail("Should not pass");
        } catch (MismatchedInputException e) {
    ...
    public void testRequiredViaParameter2591() throws Exception
    {
        ...
        try {
            /*LoginUserResponse resp =*/ MAPPER.readValue(VPackUtils.toVPack(input), LoginUserResponse.class);
            fail("Shoud not pass");
        } catch (MismatchedInputException e) {
    ...
    void testCreatorFallback2438() throws Exception {
        ...
        // not setter or field
        Creator2438 bean = MAPPER.readValue(VPackUtils.toVPack(a2q("{'value':1, 'value':2}")),
                Creator2438.class);
    ...
    void testCreatorWithReadOnly4119() throws Exception {
        Bean4119 bean = MAPPER.readerFor(Bean4119.class)
                .readValue(VPackUtils.toVPack(a2q("{'foo':'a', 'bar':'b'}")));
        assertNotNull(bean);
