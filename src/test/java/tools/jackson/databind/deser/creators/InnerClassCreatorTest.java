...
public class InnerClassCreatorTest
{
    ...
    {
        String ser = VPackUtils.toJson(MAPPER.writeValueAsBytes(new Something1501(false)));
        try {
            MAPPER.readValue(VPackUtils.toVPack(ser), Something1501.class);
    ...
    {
        String ser = VPackUtils.toJson(MAPPER.writeValueAsBytes(new Something1502(null)));

        // 21-Sep-2017, tatu: For some reason with 3.x this DOES pass (or maybe
        //    more accurately somehow with 2.x it doesn't?).
        /*
        try {
            MAPPER.readValue(VPackUtils.toVPack(ser), Something1502.class);
            fail("Should not pass");
        } catch (InvalidDefinitionException e) {
            verifyException(e, "Cannot construct instance");
        ...
        */
        Something1502 result = MAPPER.readValue(VPackUtils.toVPack(ser), Something1502.class);
    ...
    {
        String ser = VPackUtils.toJson(MAPPER.writeValueAsBytes(new Outer1503()));
        Outer1503 result = MAPPER.readValue(VPackUtils.toVPack(ser), Outer1503.class);
        assertNotNull(result);
    }
}
