...
public class LazyIgnoralForNumbers3730Test
{
    ...
    public void testIgnoreBigInteger() throws Exception
    {
        try (MockedStatic<NumberInput> mocked = mockStatic(NumberInput.class)) {
            ...
            ExtractFieldsNoDefaultConstructor3730 ef =
                    MAPPER.readValue(VPackUtils.toVPack(json), ExtractFieldsNoDefaultConstructor3730.class);
            assertNotNull(ef);
            // Ok but then let's ensure method IS called, if field is actually mapped,
            // first to Number
            try {
                Object ob = STRICT_MAPPER.readValue(VPackUtils.toVPack(json), UnwrappedWithNumber.class);
                fail("Should throw exception with mocking: instead got: "+VPackUtils.toJson(MAPPER.writeValueAsBytes(ob)));
    ...
    public void testIgnoreFPValuesDefault() throws Exception
    {
        try (MockedStatic<NumberInput> mocked = mockStatic(NumberInput.class)) {
            ...
            // First, to "Number"
            try {
                STRICT_MAPPER.readValue(VPackUtils.toVPack(json), UnwrappedWithNumber.class);
            ...
            try {
                STRICT_MAPPER.readValue(VPackUtils.toVPack(json), UnwrappedWithDouble.class);
                fail("Should throw exception with mocking!");
            } catch (JacksonException e) {
                e.printStackTrace();
                verifyMockException(e, MOCK_MSG);
    ...
    public void testIgnoreFPValuesBigDecimal() throws Exception
    {
        try (MockedStatic<NumberInput> mock = mockStatic(NumberInput.class)) {
            ...
            final String json = genJson("0.25");
            ExtractFieldsNoDefaultConstructor3730 ef =
                    reader.readValue(VPackUtils.toVPack(genJson(json)));
            assertNotNull(ef);
            ...
            // First to Number
            try {
                reader.forType(UnwrappedWithNumber.class).readValue(VPackUtils.toVPack(json));
                fail("Should throw exception with mocking!");
            } catch (JacksonException e) {
                verifyMockException(e, MOCK_MSG);
            }
            ...
            // 01-Feb-2023, tatu: Not quite working, yet:
            try {
                reader.forType(UnwrappedWithBigDecimal.class).readValue(VPackUtils.toVPack(json));
                fail("Should throw exception with mocking!");
            } catch (JacksonException e) {
                verifyMockException(e, MOCK_MSG);
            }
