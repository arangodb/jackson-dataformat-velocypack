...
{
    final static ObjectMapper MAPPER = vpackMapperBuilder()
    ...
    public void testObjectNodeUpdateValue() throws Exception
    {
        ...
        assertSame(base,
                ...
                .readValue(VPackUtils.toVPack(a2q("{'second':'bar', 'third':5, 'fourth':true}"))));
    ...
    {
        ObjectNodeWrapper w = MAPPER.readValue(VPackUtils.toVPack(a2q("{'props':{'stuff':'xyz'}}")),
...
"{'props':{'value':true, 'extra':25.5, 'array' : [ 3 ]}}"))));
    ...
    public void testArrayNodeUpdateValue() throws Exception
    {
        ...
        assertSame(base,
                MAPPER.readerForUpdating(base)
                .readValue(VPackUtils.toVPack(a2q("['second',false,null]"))));
        assertEquals(4, base.size());
    ...
    {
        ArrayNodeWrapper w = MAPPER.readValue(VPackUtils.toVPack(a2q("{'list':[456,true,{},  [], 'foo']}")),
    ...
    {
        JsonNode src = MAPPER.readTree(VPackUtils.toVPack(a2q("{'test':{}}")));
        JsonNode update = MAPPER.readTree(VPackUtils.toVPack(a2q("{'test':null}")));
    ...
    {
        JsonNode src = MAPPER.readTree(VPackUtils.toVPack(a2q("{'test':{}}")));
        JsonNode update = MAPPER.readTree(VPackUtils.toVPack(a2q("{'test':123}")));
    ...
    {
        JsonNode src = MAPPER.readTree(VPackUtils.toVPack(a2q("{'test':[]}")));
        JsonNode update = MAPPER.readTree(VPackUtils.toVPack(a2q("{'test':'NA'}")));
    ...
    public void testObjectDeepMerge3122() throws Exception
    {
        ...
        JsonNode node1 = MAPPER.readTree(VPackUtils.toVPack(a2q("{'root':{'a':'aaa','foo':'hello'}}")));
        assertEquals(2, node1.path("root").size());

        JsonNode node2 = MAPPER.readerForUpdating(node1)
                .readValue(VPackUtils.toVPack(jsonToMerge));
        assertSame(node1, node2);
        assertEquals(3, node1.path("root").size());

        node1 = MAPPER.readTree(VPackUtils.toVPack(a2q("{'root':{'a':'aaa','foo':'hello'}}")));
        JsonNode node3 = MAPPER.readerForUpdating(node1)
                .readTree(VPackUtils.toVPack(jsonToMerge));
        assertSame(node1, node3);
        assertEquals(3, node1.path("root").size());

        node1 = MAPPER.readTree(VPackUtils.toVPack(a2q("{'root':{'a':'aaa','foo':'hello'}}")));
        JsonNode node4 = MAPPER.readerForUpdating(node1)
                .readTree(vpackBytes(jsonToMerge));
        assertSame(node1, node4);
        assertEquals(3, node1.path("root").size());

        // And finally variant passing `JsonParser`
        try (JsonParser p = MAPPER.createParser(VPackUtils.toVPack(jsonToMerge))) {
