...
public class PropertyMergeTest
{
    ...
    {
        Config config = MAPPER.readValue(VPackUtils.toVPack(a2q("{'loc':{'b':3}}")), Config.class);
        assertEquals(1, config.loc.a);
        assertEquals(3, config.loc.b);

        config = MAPPER.readerForUpdating(new Config(5, 7))
                .readValue(VPackUtils.toVPack(a2q("{'loc':{'b':2}}")));
    ...
    public void testBeanMergingViaType() throws Exception
    {
        ...
        NonMergeConfig config = MAPPER.readValue(VPackUtils.toVPack(a2q("{'loc':{'a':3}}")), NonMergeConfig.class);
        ...
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        config = mapper.readValue(VPackUtils.toVPack(a2q("{'loc':{'a':3}}")), NonMergeConfig.class);
        assertEquals(3, config.loc.a);
    ...
    public void testBeanMergingViaGlobal() throws Exception
    {
        ...
        FiveMinuteUser user = mapper.readerFor(FiveMinuteUser.class)
                ...
                .readValue(VPackUtils.toVPack(a2q("{'name':{'last':'Brown'}}")));
    ...
    {
        NoSetterConfig config = MAPPER.readValue(VPackUtils.toVPack(a2q("{'value':{'b':99}}")),
    ...
    public void testBeanMergeUsingConstructors() throws Exception {
        ...
        ConstructorArgsPojo result = vpackMapperBuilder()
                .defaultMergeable(true)
                .build()
                .readerForUpdating(input)
                .readValue(VPackUtils.toVPack(a2q("{'mergeableBean': {'foo': 'newFoo'}}")));
    ...
    public void testBeanAsArrayMerging() throws Exception
    {
        ...
        try {
            MAPPER.readerForUpdating(input)
                    ...
                    .readValue(VPackUtils.toVPack("[9, 8, 14]"));
        ...
        try {
            MAPPER.readerForUpdating(input)
                .readValue(VPackUtils.toVPack("\"blob\""));
            fail("Should not pass");
        } catch (MismatchedInputException e) {
    ...
    {
        MergedReference result = MAPPER.readValue(VPackUtils.toVPack(a2q("{'value':'override'}")),
    ...
    public void testInvalidPropertyMerge() throws Exception
    {
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        try {
            mapper.readValue(VPackUtils.toVPack("{\"value\":3}"), CantMergeInts.class);
            fail("Should not pass");
