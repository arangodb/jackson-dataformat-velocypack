...
public class VPackNodeFactoryTest extends NodeTestBase
{
    ...
    public void testSortingObjectNode() throws Exception
    {
       ...
       assertEquals(SIMPLE_INPUT,
               VPackUtils.toJson(MAPPER.writeValueAsBytes(MAPPER.readTree(VPackUtils.toVPack(SIMPLE_INPUT)))));

       // But can change easily
       ObjectMapper mapper = VPackMapper.builder()
               .nodeFactory(new SortingNodeFactory())
               .build();
       JsonNode sort = mapper.readTree(VPackUtils.toVPack(SIMPLE_INPUT));
       assertEquals("{\"a\":1,\"b\":2}", VPackUtils.toJson(MAPPER.writeValueAsBytes(sort)));
       ...
       assertEquals(BIGGER_OUTPUT,
               VPackUtils.toJson(MAPPER.writeValueAsBytes(mapper.readTree(VPackUtils.toVPack(BIGGER_INPUT)))));
   }
   ...
   public void testBigDecimalNormalization() throws Exception
   {
       ...
       ObjectMapper normMapper = VPackMapper.builder()
               ...
               .build();
       JsonNode n3 = normMapper.readTree(VPackUtils.toVPack(String.valueOf(NON_NORMALIZED)));
