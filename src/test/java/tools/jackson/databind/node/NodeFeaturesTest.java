...
public class NodeFeaturesTest extends DatabindTestUtil
{
    ...
    public void testReadNulls() throws Exception
    {
        ...
        assertEquals(DOC_WITH_NULL, READER.readTree(VPackUtils.toVPack(JSON_WITH_NULL)));
        ...
        assertEquals(DOC_EMPTY, r.readTree(VPackUtils.toVPack(JSON_WITH_NULL)));

        // but also verify we can "reset" reader's behavior
        ObjectReader r2 = r.with(JsonNodeFeature.READ_NULL_PROPERTIES);
        assertEquals(DOC_WITH_NULL, r2.readTree(VPackUtils.toVPack(JSON_WITH_NULL)));
    ...
    public void testWriteNulls() throws Exception
    {
        ...
        assertEquals(JSON_WITH_NULL, VPackUtils.toJson(WRITER.writeValueAsBytes(DOC_WITH_NULL)));
        ...
        assertEquals(JSON_EMPTY, VPackUtils.toJson(w.writeValueAsBytes(DOC_WITH_NULL)));

        // but also verify we can "reset" writer's behavior
        ObjectWriter w2 = w.with(JsonNodeFeature.WRITE_NULL_PROPERTIES);
        assertEquals(JSON_WITH_NULL, VPackUtils.toJson(w2.writeValueAsBytes(DOC_WITH_NULL)));
        ...
        doc.putNull("b");
        doc.put("c", true);
        assertEquals(a2q("{'a':1,'c':true}"), VPackUtils.toJson(w.writeValueAsBytes(doc)));
    }
    ...
    public void testWriteSortedProperties() throws Exception
    {
        ...
        assertEquals(a2q("{'b':2,'c':3,'a':1}"), VPackUtils.toJson(WRITER.writeValueAsBytes(doc)));
        ...
        ObjectMapper sortingMapper = VPackMapper.builder()
                ...
                .build();
        assertEquals(SORTED, VPackUtils.toJson(sortingMapper.writeValueAsBytes(doc)));
        ...
        assertEquals(SORTED, VPackUtils.toJson(w2.writeValueAsBytes(doc)));
    }
    ...
    public void testBigDecimalForJsonNodeFeature() throws Exception {
        ...
        mapper = VPackMapper.builder()
                ...
                .build();
        assertEquals(expectedBigDecimal, mapper.readTree(VPackUtils.toVPack(JSON)).decimalValue());
        ...
        mapper = VPackMapper.builder()
                ...
                .build();
        assertEquals(expectedDoubleLossy, mapper.readTree(VPackUtils.toVPack(JSON)).decimalValue());
