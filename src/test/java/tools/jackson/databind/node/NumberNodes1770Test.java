...
public class NumberNodes1770Test extends DatabindTestUtil
{
    ...
    public void testBigDecimalCoercion() throws Exception
    {
        ...
        final JsonNode jsonNode = newVPackMapper().reader()
            .with(DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS)
            .readTree(VPackUtils.toVPack(value));
    ...
    public void testBigDecimalCoercionInf() throws Exception
    {
        ...
        // .enable(VPackReadFeature.ALLOW_NON_NUMERIC_NUMBERS)
                .build();
        final JsonNode jsonNode = new VPackMapper(factory).reader()
                .with(DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS)
                .readTree(VPackUtils.toVPack(value));
    ...
    private JsonNode _tryBigDecimalCoercionNaNWithOption(boolean isEnabled) throws Exception
    {
        ...
        // .enable(VPackReadFeature.ALLOW_NON_NUMERIC_NUMBERS)
                .build();
        final ObjectReader reader = new VPackMapper(factory)
                .reader()
                .with(DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS);
        ...
        // depending on option
        return isEnabled
                ? reader.with(JsonNodeFeature.FAIL_ON_NAN_TO_BIG_DECIMAL_COERCION).readTree(VPackUtils.toVPack(value))
                : reader.without(JsonNodeFeature.FAIL_ON_NAN_TO_BIG_DECIMAL_COERCION).readTree(VPackUtils.toVPack(value));
    }
}
