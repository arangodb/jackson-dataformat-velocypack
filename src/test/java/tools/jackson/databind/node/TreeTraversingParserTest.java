    ...
    extends DatabindTestUtil
{
    ...
    public void testSimple() throws Exception
    {
        ...
        JsonNode tree = MAPPER.readTree(VPackUtils.toVPack(JSON));
    ...
    {
        // For convenience, parse tree from JSON first
        JsonParser p = MAPPER.readTree(VPackUtils.toVPack("[]")).traverse(ObjectReadContext.empty());
        assertToken(JsonToken.START_ARRAY, p.nextToken());
        assertToken(JsonToken.END_ARRAY, p.nextToken());
        p.close();

        p = MAPPER.readTree(VPackUtils.toVPack("[[]]")).traverse(ObjectReadContext.empty());
        assertToken(JsonToken.START_ARRAY, p.nextToken());
        assertToken(JsonToken.START_ARRAY, p.nextToken());
        assertToken(JsonToken.END_ARRAY, p.nextToken());
        assertToken(JsonToken.END_ARRAY, p.nextToken());
        ...
        p = MAPPER.readTree(VPackUtils.toVPack("[[ 12.1 ]]")).traverse(ObjectReadContext.empty());
        assertToken(JsonToken.START_ARRAY, p.nextToken());
        assertToken(JsonToken.START_ARRAY, p.nextToken());
        assertToken(JsonToken.VALUE_NUMBER_FLOAT, p.nextToken());
        assertToken(JsonToken.END_ARRAY, p.nextToken());
    ...
    {
        JsonNode tree = MAPPER.readTree(VPackUtils.toVPack(SAMPLE_DOC_JSON_SPEC));
        JsonParser p = tree.traverse(ObjectReadContext.empty());
    ...
    public void testNumberOverflowInt() throws Exception
    {
        ...
        try (final JsonParser p = MAPPER.readTree(VPackUtils.toVPack("[ "+tooBig+" ]")).traverse(ObjectReadContext.empty())) {
            assertToken(JsonToken.START_ARRAY, p.nextToken());
            assertToken(JsonToken.VALUE_NUMBER_INT, p.nextToken());
        ...
        try (final JsonParser p = MAPPER.readTree(VPackUtils.toVPack("{ \"value\" : "+tooBig+" }")).traverse(ObjectReadContext.empty())) {
            assertToken(JsonToken.START_OBJECT, p.nextToken());
            assertToken(JsonToken.PROPERTY_NAME, p.nextToken());
            assertToken(JsonToken.VALUE_NUMBER_INT, p.nextToken());
            assertEquals(NumberType.LONG, p.getNumberType());
        ...
        try (final JsonParser p = MAPPER.readTree(VPackUtils.toVPack("[ "+tooBig2+" ]")).traverse(ObjectReadContext.empty())) {
            assertToken(JsonToken.START_ARRAY, p.nextToken());
            assertToken(JsonToken.VALUE_NUMBER_FLOAT, p.nextToken());
            assertEquals(NumberType.DOUBLE, p.getNumberType());
            try {
    ...
    public void testNumberOverflowLong() throws Exception
    {
        ...
        for (long okValue : okValues) {
            try (final JsonParser p = MAPPER.readTree(VPackUtils.toVPack("{ \"value\" : "+okValue+" }")).traverse(ObjectReadContext.empty())) {
                assertToken(JsonToken.START_OBJECT, p.nextToken());
                assertToken(JsonToken.PROPERTY_NAME, p.nextToken());
                assertToken(JsonToken.VALUE_NUMBER_INT, p.nextToken());
                assertEquals(NumberType.LONG, p.getNumberType());
