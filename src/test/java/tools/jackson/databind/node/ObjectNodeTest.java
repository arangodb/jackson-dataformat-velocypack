    ...
    extends DatabindTestUtil
{
    ...
    {
        Data w = MAPPER.readValue(VPackUtils.toVPack("{}"), Data.class);
        assertNotNull(w);
    }
    ...
    public void testValidWithObject() throws Exception
    {
        ...
        assertEquals("{}", VPackUtils.toJson(MAPPER.writeValueAsBytes(root)));
        JsonNode child = root.withObject("/prop");
        assertInstanceOf(ObjectNode.class, child);
        assertEquals("{\"prop\":{}}", VPackUtils.toJson(MAPPER.writeValueAsBytes(root)));
    }
    ...
    public void testValidWithArray() throws Exception
    {
        ...
        assertEquals("{\"arr\":[]}", VPackUtils.toJson(MAPPER.writeValueAsBytes(root)));
    }
    ...
    {
        JsonNode root = MAPPER.readTree(VPackUtils.toVPack("{ \"results\" : { \"a\" : 3 } }"));
        assertTrue(root.isObject());
    ...
    public void testNonEmptySerialization() throws Exception
    {
        ObNodeWrapper w = new ObNodeWrapper(MAPPER.createObjectNode()
                .put("a", 3));
        assertEquals("{\"node\":{\"a\":3}}", VPackUtils.toJson(MAPPER.writeValueAsBytes(w)));
        w = new ObNodeWrapper(MAPPER.createObjectNode());
        assertEquals("{}", VPackUtils.toJson(MAPPER.writeValueAsBytes(w)));
    }
...
//        System.out.println("json: "+json);
        ...
        ObjectNode de1 = MAPPER.readValue(VPackUtils.toVPack(json), ObjectNode.class);  // this works
//        System.out.println("Deserialized to ObjectNode: "+de1);
        assertNotNull(de1);

        MyValue de2 = MAPPER.readValue(VPackUtils.toVPack(json), MyValue.class);  // but this throws exception
//        System.out.println("Deserialized to MyValue: "+de2);
    ...
    {
        try {
            MAPPER.readValue(VPackUtils.toVPack("[ 1, 2, 3 ]"), ObjectNode.class);
    ...
    public void testPropertiesTraversal() throws Exception
    {
        ...
        JsonNode n = MAPPER.readTree(VPackUtils.toVPack(a2q(
                "{ 'a':1, 'b':true,'c':'stuff'}")));
    ...
    private ObjectNode _objNode(String json) throws Exception {
        ...
        try (JsonParser p = MAPPER.createParser(VPackUtils.toVPack(a2q(json)))) {
