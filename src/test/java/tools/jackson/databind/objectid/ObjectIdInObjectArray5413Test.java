...
public class ObjectIdInObjectArray5413Test extends DatabindTestUtil
{
    ...
    public void testForwardReferenceResolution()
    {
        ...
        final String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(draw));
        draw = MAPPER.readValue(VPackUtils.toVPack(json), Draw.class);
        assertNotNull(draw);
    ...
    {
        Draw draw = MAPPER.readValue(VPackUtils.toVPack(a2q("{'AShapes':[ null ], 'points': [ null ]}")),
                Draw.class);
    ...
    public void testForwardReferenceInArray() throws Exception {
        String json = "{\"employees\":["
                ...
                + "]}";
        ArrayCompany company = MAPPER.readValue(VPackUtils.toVPack(json), ArrayCompany.class);
    ...
    public void testForwardReferenceInQueue() throws Exception {
        String json = "{\"employees\":["
                ...
                + "{\"id\":2,\"name\":\"Second\",\"manager\":1,\"reports\":[]}"
                + "]}";
        ArrayBlockingQueueCompany company = MAPPER.readValue(VPackUtils.toVPack(json), ArrayBlockingQueueCompany.class);
