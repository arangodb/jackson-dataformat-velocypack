...
//        _verifyNoCoercion(listType);
        try {
            VANILLA_MAPPER.readerFor(listType).readValue(VPackUtils.toVPack(JSON_EMPTY));
...
//            verifyException(e, "Cannot coerce empty String");
    ...
    private void _verifyNoCoercion(JavaType targetType) throws Exception {
        try {
            VANILLA_MAPPER.readerFor(targetType).readValue(VPackUtils.toVPack(JSON_EMPTY));
            fail("Should not pass");
        } catch (DatabindException e) {
...
//            verifyException(e, "Cannot deserialize value of type");
    ...
    private <T> T _readWithCoercion(Class<?> targetType) throws Exception {
        return COERCING_MAPPER.readerFor(targetType).readValue(VPackUtils.toVPack(JSON_EMPTY));
    }
