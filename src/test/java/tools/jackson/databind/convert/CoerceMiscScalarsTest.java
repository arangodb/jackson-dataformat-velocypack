...
public class CoerceMiscScalarsTest
{
    ...
    private final ObjectMapper MAPPER_EMPTY_TO_EMPTY = vpackMapperBuilder()
            .withCoercionConfigDefaults(cfg ->
                cfg.setCoercion(CoercionInputShape.EmptyString, CoercionAction.AsEmpty))
            .build();

    private final ObjectMapper MAPPER_EMPTY_TO_TRY_CONVERT = vpackMapperBuilder()
            .withCoercionConfigDefaults(cfg ->
                cfg.setCoercion(CoercionInputShape.EmptyString, CoercionAction.TryConvert))
            .build();

    private final ObjectMapper MAPPER_EMPTY_TO_NULL = vpackMapperBuilder()
            .withCoercionConfigDefaults(cfg ->
                cfg.setCoercion(CoercionInputShape.EmptyString, CoercionAction.AsNull))
            .build();

    private final ObjectMapper MAPPER_EMPTY_TO_FAIL = vpackMapperBuilder()
            .withCoercionConfigDefaults(cfg ->
    ...
    public void testUUIDCoercions() throws Exception
    {
        ...
        ObjectMapper failMapper = vpackMapperBuilder()
    ...
    public void testStringBuilderCoercions() throws Exception
    {
        ...
        _checkEmptyStringBuilder(DEFAULT_MAPPER.readValue(VPackUtils.toVPack(JSON_EMPTY), StringBuilder.class));
        _checkEmptyStringBuilder(MAPPER_EMPTY_TO_EMPTY.readValue(VPackUtils.toVPack(JSON_EMPTY), StringBuilder.class));
        _checkEmptyStringBuilder(MAPPER_EMPTY_TO_TRY_CONVERT.readValue(VPackUtils.toVPack(JSON_EMPTY), StringBuilder.class));
        _checkEmptyStringBuilder(MAPPER_EMPTY_TO_NULL.readValue(VPackUtils.toVPack(JSON_EMPTY), StringBuilder.class));
        // and even alleged failure should not result in that since it's not coercion
        _checkEmptyStringBuilder(MAPPER_EMPTY_TO_FAIL.readValue(VPackUtils.toVPack(JSON_EMPTY), StringBuilder.class));
    }
...
//        _testScalarEmptyToEmpty(MAPPER_EMPTY_TO_EMPTY, Calendar.class, emptyCal);
    ...
    {
        assertNull(mapper.readerFor(target).readValue(VPackUtils.toVPack(JSON_EMPTY)));
    }

    private void _testScalarEmptyToEmpty(ObjectMapper mapper,
            Class<?> target, Object emptyValue) throws Exception
    {
        Object result = mapper.readerFor(target).readValue(VPackUtils.toVPack(JSON_EMPTY));
        if (result == null) {
    ...
    private void _verifyScalarToFail(ObjectMapper mapper, Class<?> target) throws Exception
    {
        try {
            /*Object result =*/ mapper.readerFor(target)
                .readValue(VPackUtils.toVPack(JSON_EMPTY));
