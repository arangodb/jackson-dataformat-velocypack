...
public class CoerceJDKScalarsTest
{
    ...
    private final ObjectMapper COERCING_MAPPER = vpackMapperBuilder()
            .enable(MapperFeature.ALLOW_COERCION_OF_SCALARS)
            .build();

    private final ObjectMapper NOT_COERCING_MAPPER = vpackMapperBuilder()
    ...
    {
        try {
            NOT_COERCING_MAPPER.readerFor(type).readValue(VPackUtils.toVPack("\"\""));
    ...
    public void testStringToNumbersCoercionOk() throws Exception
    {
        ...
        AtomicBoolean ab = COERCING_MAPPER.readValue(VPackUtils.toVPack(q("true")), AtomicBoolean.class);
        assertNotNull(ab);
        assertTrue(ab.get());
    }
    ...
    private void _verifyRootStringCoerceFail(String unquotedValue, Class<?> type) throws IOException
    {
        ...
        try (JsonParser p = NOT_COERCING_MAPPER.createParser(VPackUtils.toVPack(input))) {
