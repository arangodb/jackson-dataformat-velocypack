...
public class CoerceToBooleanTest
{
    ...
    private final ObjectMapper DEFAULT_MAPPER = vpackMapperBuilder()
            .disable(DeserializationFeature.FAIL_ON_NULL_FOR_PRIMITIVES)
            .build();

    private final ObjectMapper LEGACY_NONCOERCING_MAPPER = vpackMapperBuilder()
    ...
    private final ObjectMapper MAPPER_INT_TO_EMPTY = vpackMapperBuilder()
            .withCoercionConfig(LogicalType.Boolean, cfg ->
                cfg.setCoercion(CoercionInputShape.Integer, CoercionAction.AsEmpty))
            .build();

    private final ObjectMapper MAPPER_INT_TRY_CONVERT = vpackMapperBuilder()
            .withCoercionConfig(LogicalType.Boolean, cfg ->
                cfg.setCoercion(CoercionInputShape.Integer, CoercionAction.TryConvert))
            .build();

    private final ObjectMapper MAPPER_INT_TO_NULL = vpackMapperBuilder()
            .withCoercionConfig(LogicalType.Boolean, cfg ->
    ...
    public void testEmptyStringFailForBooleanPrimitive() throws IOException
    {
        ...
        try {
            reader.readValue(VPackUtils.toVPack(a2q("{'booleanValue':''}")));
            ...
            String unquotedValue, Class<?> type) throws IOException
    {
        ...
        try (JsonParser p = nonCoercingMapper.createParser(VPackUtils.toVPack((input)))) {
            _verifyStringCoerceFail(nonCoercingMapper, p, unquotedValue, type);
        }
        final byte[] inputBytes = vpackBytes(input);
    ...
    public void testIntToBooleanCoercionSuccessPojo() throws Exception
    {
        ...
        p = r.readValue(VPackUtils.toVPack(DOC_WITH_0));
        assertFalse(p.value);
        p = r.readValue(vpackBytes(DOC_WITH_0));
        assertFalse(p.value);

        p = r.readValue(VPackUtils.toVPack(DOC_WITH_1));
        assertTrue(p.value);
        p = r.readValue(vpackBytes(DOC_WITH_1));
        assertTrue(p.value);
    }
    ...
    public void testIntToBooleanCoercionSuccessRoot() throws Exception
    {
        ...
        assertEquals(Boolean.FALSE, br.readValue(VPackUtils.toVPack(" 0")));
        assertEquals(Boolean.FALSE, br.readValue(vpackBytes(" 0")));
        assertEquals(Boolean.TRUE, br.readValue(VPackUtils.toVPack(" -1")));
        assertEquals(Boolean.TRUE, br.readValue(vpackBytes(" -1")));

        final ObjectReader atomicR = DEFAULT_MAPPER.readerFor(AtomicBoolean.class);

        AtomicBoolean ab;

        ab = atomicR.readValue(VPackUtils.toVPack(" 0"));
        ab = atomicR.readValue(vpackBytes(" 0"));
        assertFalse(ab.get());

        ab = atomicR.readValue(VPackUtils.toVPack(" 111"));
        assertTrue(ab.get());
        ab = atomicR.readValue(vpackBytes(" 111"));
        assertTrue(ab.get());
    }
    ...
    public void testLongToBooleanCoercionOk() throws Exception
    {
        ...
        BooleanWrapper b = DEFAULT_MAPPER.readValue(VPackUtils.toVPack("{\"primitive\" : "+value+", \"wrapper\":"+value+", \"ctor\":"+value+"}"),
        ...
        b = DEFAULT_MAPPER.readValue(VPackUtils.toVPack("{\"primitive\" : 0 , \"wrapper\":0, \"ctor\":0}"),
                BooleanWrapper.class);
        assertEquals(Boolean.FALSE, b.wrapper);
        assertFalse(b.primitive);
        assertEquals(Boolean.FALSE, b.ctor);

        boolean[] boo = DEFAULT_MAPPER.readValue(VPackUtils.toVPack("[ 0, 15, \"\", \"false\", \"True\" ]"),
                boolean[].class);
    ...
    {
        assertNull(MAPPER_INT_TO_NULL.readValue(VPackUtils.toVPack("0"), Boolean.class));
        assertNull(MAPPER_INT_TO_NULL.readValue(VPackUtils.toVPack("1"), Boolean.class));

        // but due to coercion to `boolean`, cannot return null here -- however,
        // goes "1 -> false (no null for primitive) -> Boolean.FALSE
        assertEquals(Boolean.FALSE, MAPPER_INT_TO_NULL.readValue(VPackUtils.toVPack("0"), Boolean.TYPE));
        assertEquals(Boolean.FALSE, MAPPER_INT_TO_NULL.readValue(VPackUtils.toVPack("1"), Boolean.TYPE));

        // As to AtomicBoolean: that type itself IS nullable since it's of LogicalType.Boolean so
        assertNull(MAPPER_INT_TO_NULL.readValue(VPackUtils.toVPack("0"), AtomicBoolean.class));
        assertNull(MAPPER_INT_TO_NULL.readValue(VPackUtils.toVPack("1"), AtomicBoolean.class));

        BooleanPOJO p;
        p = MAPPER_INT_TO_NULL.readValue(VPackUtils.toVPack(DOC_WITH_0), BooleanPOJO.class);
        assertFalse(p.value);
        p = MAPPER_INT_TO_NULL.readValue(VPackUtils.toVPack(DOC_WITH_1), BooleanPOJO.class);
        assertFalse(p.value);
    }
    ...
    public void testIntToEmptyCoercion() throws Exception
    {
        ...
        assertEquals(Boolean.FALSE, MAPPER_INT_TO_EMPTY.readValue(VPackUtils.toVPack("0"), Boolean.class));
        assertEquals(Boolean.FALSE, MAPPER_INT_TO_EMPTY.readValue(VPackUtils.toVPack("1"), Boolean.class));

        assertEquals(Boolean.FALSE, MAPPER_INT_TO_EMPTY.readValue(VPackUtils.toVPack("0"), Boolean.TYPE));
        assertEquals(Boolean.FALSE, MAPPER_INT_TO_EMPTY.readValue(VPackUtils.toVPack("1"), Boolean.TYPE));

        AtomicBoolean ab;
        ab = MAPPER_INT_TO_EMPTY.readValue(VPackUtils.toVPack("0"), AtomicBoolean.class);
        assertFalse(ab.get());
        ab = MAPPER_INT_TO_EMPTY.readValue(VPackUtils.toVPack("1"), AtomicBoolean.class);
        assertFalse(ab.get());

        BooleanPOJO p;
        p = MAPPER_INT_TO_EMPTY.readValue(VPackUtils.toVPack(DOC_WITH_0), BooleanPOJO.class);
        assertFalse(p.value);
        p = MAPPER_INT_TO_EMPTY.readValue(VPackUtils.toVPack(DOC_WITH_1), BooleanPOJO.class);
        assertFalse(p.value);
    }
    ...
    public void testIntToTryCoercion() throws Exception
    {
        ...
        assertEquals(Boolean.FALSE, MAPPER_INT_TRY_CONVERT.readValue(VPackUtils.toVPack("0"), Boolean.class));
        assertEquals(Boolean.TRUE, MAPPER_INT_TRY_CONVERT.readValue(VPackUtils.toVPack("1"), Boolean.class));

        assertEquals(Boolean.FALSE, MAPPER_INT_TRY_CONVERT.readValue(VPackUtils.toVPack("0"), Boolean.TYPE));
        assertEquals(Boolean.TRUE, MAPPER_INT_TRY_CONVERT.readValue(VPackUtils.toVPack("1"), Boolean.TYPE));

        AtomicBoolean ab;
        ab = MAPPER_INT_TRY_CONVERT.readValue(VPackUtils.toVPack("0"), AtomicBoolean.class);
        assertFalse(ab.get());
        ab = MAPPER_INT_TRY_CONVERT.readValue(VPackUtils.toVPack("1"), AtomicBoolean.class);
        assertTrue(ab.get());

        BooleanPOJO p;
        p = MAPPER_INT_TRY_CONVERT.readValue(VPackUtils.toVPack(DOC_WITH_0), BooleanPOJO.class);
        assertFalse(p.value);
        p = MAPPER_INT_TRY_CONVERT.readValue(VPackUtils.toVPack(DOC_WITH_1), BooleanPOJO.class);
        assertTrue(p.value);
    }
            ...
            JsonToken tokenType, String tokenValue, Class<?> targetType) throws IOException
    {
        ...
        try {
            if (useBytes) {
                r.readValue(vpackBytes(doc));
            } else {
                r.readValue(VPackUtils.toVPack(doc));
            }
            fail("Should not have allowed coercion");
        } catch (MismatchedInputException e) {
    ...
    {
        try {
            m.readerFor(targetType).readValue(VPackUtils.toVPack(doc));
