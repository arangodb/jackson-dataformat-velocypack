...
public class CoerceIntToFloatTest
{
    ...
    private final ObjectMapper LEGACY_SCALAR_COERCION_FAIL = vpackMapperBuilder()
    ...
    {
        assertNull(MAPPER_TO_NULL.readValue(VPackUtils.toVPack("1"), Float.class));
        // `null` not possible for primitives, must use empty (aka default) value
        assertEquals(0.0f, MAPPER_TO_NULL.readValue(VPackUtils.toVPack("-2"), Float.TYPE));
        {
            FloatWrapper w = MAPPER_TO_NULL.readValue(VPackUtils.toVPack("{\"f\": -5}"), FloatWrapper.class);
            assertEquals(0.0f, w.f);
            float[] arr = MAPPER_TO_NULL.readValue(VPackUtils.toVPack("[ 2 ]"), float[].class);
            assertEquals(1, arr.length);
            assertEquals(0.0f, arr[0]);
        }

        assertNull(MAPPER_TO_NULL.readValue(VPackUtils.toVPack("-1"), Double.class));
        assertEquals(0.0d, MAPPER_TO_NULL.readValue(VPackUtils.toVPack("4"), Double.TYPE));
        {
            DoubleWrapper w = MAPPER_TO_NULL.readValue(VPackUtils.toVPack("{\"d\": 2}"), DoubleWrapper.class);
            assertEquals(0.0d, w.d);
            double[] arr = MAPPER_TO_NULL.readValue(VPackUtils.toVPack("[ -7 ]"), double[].class);
            assertEquals(1, arr.length);
            assertEquals(0.0d, arr[0]);
        }

        assertNull(MAPPER_TO_NULL.readValue(VPackUtils.toVPack("420"), BigDecimal.class));
        {
            BigDecimal[] arr = MAPPER_TO_NULL.readValue(VPackUtils.toVPack("[ 420 ]"), BigDecimal[].class);
            assertEquals(1, arr.length);
            assertNull(arr[0]);
        }
    }
    ...
    {
        assertEquals(0.0f, MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("3"), Float.class));
        assertEquals(0.0f, MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("-2"), Float.TYPE));
        {
            FloatWrapper w = MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("{\"f\": -5}"), FloatWrapper.class);
            assertEquals(0.0f, w.f);
            float[] arr = MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("[ 2 ]"), float[].class);
            assertEquals(1, arr.length);
            assertEquals(0.0f, arr[0]);
        }

        assertEquals(0.0d, MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("-1"), Double.class));
        assertEquals(0.0d, MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("-5"), Double.TYPE));
        {
            DoubleWrapper w = MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("{\"d\": 2}"), DoubleWrapper.class);
            assertEquals(0.0d, w.d);
            double[] arr = MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("[ -2 ]"), double[].class);
            assertEquals(1, arr.length);
            assertEquals(0.0d, arr[0]);
        }

        assertEquals(BigDecimal.valueOf(0), MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("3643"), BigDecimal.class));
    }
    ...
    private void assertSuccessfulIntToFloatConversionsWith(ObjectMapper objectMapper)
            throws Exception
    {
        assertEquals(3.0f, objectMapper.readValue(VPackUtils.toVPack("3"), Float.class));
        assertEquals(-2.0f, objectMapper.readValue(VPackUtils.toVPack("-2"), Float.TYPE));
        {
            FloatWrapper w = objectMapper.readValue(VPackUtils.toVPack("{\"f\": -5}"), FloatWrapper.class);
            assertEquals(-5.0f, w.f);
            float[] arr = objectMapper.readValue(VPackUtils.toVPack("[ 2 ]"), float[].class);
            assertEquals(2.0f, arr[0]);
        }

        assertEquals(-1.0d, objectMapper.readValue(VPackUtils.toVPack("-1"), Double.class));
        assertEquals(4.0d, objectMapper.readValue(VPackUtils.toVPack("4"), Double.TYPE));
        {
            DoubleWrapper w = objectMapper.readValue(VPackUtils.toVPack("{\"d\": 2}"), DoubleWrapper.class);
            assertEquals(2.0d, w.d);
            double[] arr = objectMapper.readValue(VPackUtils.toVPack("[ -2 ]"), double[].class);
            assertEquals(-2.0d, arr[0]);
        }

        BigDecimal biggie = objectMapper.readValue(VPackUtils.toVPack("423451233"), BigDecimal.class);
