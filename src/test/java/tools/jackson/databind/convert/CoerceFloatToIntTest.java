...
public class CoerceFloatToIntTest
{
    ...
    public void testLegacyDoubleToIntCoercion() throws Exception
    {
        ...
        Integer I = DEFAULT_MAPPER.readValue(VPackUtils.toVPack(" 1.25 "), Integer.class);
        assertEquals(1, I.intValue());
        {
            IntWrapper w = DEFAULT_MAPPER.readValue(VPackUtils.toVPack("{\"i\":-2.25 }"), IntWrapper.class);
            assertEquals(-2, w.i);
            int[] arr = DEFAULT_MAPPER.readValue(VPackUtils.toVPack("[ 1.25 ]"), int[].class);
            assertEquals(1, arr[0]);
        }

        Long L = DEFAULT_MAPPER.readValue(VPackUtils.toVPack(" 3.33 "), Long.class);
        assertEquals(3L, L.longValue());
        {
            LongWrapper w = DEFAULT_MAPPER.readValue(VPackUtils.toVPack("{\"l\":-2.25 }"), LongWrapper.class);
            assertEquals(-2L, w.l);
            long[] arr = DEFAULT_MAPPER.readValue(VPackUtils.toVPack("[ 1.25 ]"), long[].class);
            assertEquals(1, arr[0]);
        }

        Short S = DEFAULT_MAPPER.readValue(VPackUtils.toVPack("42.33"), Short.class);
        assertEquals(42, S.intValue());

        BigInteger biggie = DEFAULT_MAPPER.readValue(VPackUtils.toVPack("95.3"), BigInteger.class);
    ...
    {
        assertNull(MAPPER_TO_NULL.readValue(VPackUtils.toVPack("1.5"), Integer.class));
        // `null` not possible for primitives, must use empty (aka default) value
        assertEquals(Integer.valueOf(0), MAPPER_TO_NULL.readValue(VPackUtils.toVPack("1.5"), Integer.TYPE));
        {
            IntWrapper w = MAPPER_TO_NULL.readValue( VPackUtils.toVPack("{\"i\":-2.25 }"), IntWrapper.class);
            assertEquals(0, w.i);
            int[] ints = MAPPER_TO_NULL.readValue(VPackUtils.toVPack("[ 2.5 ]"), int[].class);
            assertEquals(1, ints.length);
            assertEquals(0, ints[0]);
        }

        assertNull(MAPPER_TO_NULL.readValue(VPackUtils.toVPack("2.5"), Long.class));
        assertEquals(Long.valueOf(0L), MAPPER_TO_NULL.readValue(VPackUtils.toVPack("-4.25"), Long.TYPE));
        {
            LongWrapper w = MAPPER_TO_NULL.readValue( VPackUtils.toVPack("{\"l\":-2.25 }"), LongWrapper.class);
            assertEquals(0L, w.l);
            long[] l = MAPPER_TO_NULL.readValue(VPackUtils.toVPack("[ 2.5 ]"), long[].class);
            assertEquals(1, l.length);
            assertEquals(0L, l[0]);
        }

        assertNull(MAPPER_TO_NULL.readValue(VPackUtils.toVPack("2.5"), Short.class));
        assertEquals(Short.valueOf((short) 0), MAPPER_TO_NULL.readValue(VPackUtils.toVPack("-4.25"), Short.TYPE));
        {
            short[] s = MAPPER_TO_NULL.readValue(VPackUtils.toVPack("[ 2.5 ]"), short[].class);
            assertEquals(1, s.length);
            assertEquals((short) 0, s[0]);
        }

        assertNull(MAPPER_TO_NULL.readValue(VPackUtils.toVPack("2.5"), Byte.class));
        assertEquals(Byte.valueOf((byte) 0), MAPPER_TO_NULL.readValue(VPackUtils.toVPack("-4.25"), Byte.TYPE));
        {
            byte[] arr = MAPPER_TO_NULL.readValue(VPackUtils.toVPack("[ 2.5 ]"), byte[].class);
            assertEquals(1, arr.length);
            assertEquals((byte) 0, arr[0]);
        }

        assertNull(MAPPER_TO_NULL.readValue(VPackUtils.toVPack("2.5"), BigInteger.class));
        {
            BigInteger[] arr = MAPPER_TO_NULL.readValue(VPackUtils.toVPack("[ 2.5 ]"), BigInteger[].class);
            assertEquals(1, arr.length);
            assertNull(arr[0]);
        }
    }
    ...
    {
        assertEquals(Integer.valueOf(0), MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("1.2"), Integer.class));
        assertEquals(Integer.valueOf(0), MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("1.5"), Integer.TYPE));
        {
            IntWrapper w = MAPPER_TO_EMPTY.readValue( VPackUtils.toVPack("{\"i\":-2.25 }"), IntWrapper.class);
            assertEquals(0, w.i);
            int[] ints = MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("[ 2.5 ]"), int[].class);
            assertEquals(1, ints.length);
            assertEquals(0, ints[0]);
        }

        assertEquals(Long.valueOf(0), MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("1.2"), Long.class));
        assertEquals(Long.valueOf(0), MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("1.5"), Long.TYPE));
        {
            LongWrapper w = MAPPER_TO_EMPTY.readValue( VPackUtils.toVPack("{\"l\":-2.25 }"), LongWrapper.class);
            assertEquals(0L, w.l);
            long[] l = MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("[ 2.5 ]"), long[].class);
            assertEquals(1, l.length);
            assertEquals(0L, l[0]);
        }

        assertEquals(Short.valueOf((short)0), MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("1.2"), Short.class));
        assertEquals(Short.valueOf((short) 0), MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("1.5"), Short.TYPE));

        assertEquals(Byte.valueOf((byte)0), MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("1.2"), Byte.class));
        assertEquals(Byte.valueOf((byte) 0), MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("1.5"), Byte.TYPE));

        assertEquals(BigInteger.valueOf(0L), MAPPER_TO_EMPTY.readValue(VPackUtils.toVPack("124.5"), BigInteger.class));
    }
    ...
    {
        assertEquals(Integer.valueOf(1), MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("1.2"), Integer.class));
        assertEquals(Integer.valueOf(3), MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("3.4"), Integer.TYPE));
        {
            IntWrapper w = MAPPER_TRY_CONVERT.readValue( VPackUtils.toVPack("{\"i\":-2.25 }"), IntWrapper.class);
            assertEquals(-2, w.i);
            int[] ints = MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("[ 22.10 ]"), int[].class);
            assertEquals(1, ints.length);
            assertEquals(22, ints[0]);
        }

        assertEquals(Long.valueOf(1), MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("1.2"), Long.class));
        assertEquals(Long.valueOf(1), MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("1.5"), Long.TYPE));
        {
            LongWrapper w = MAPPER_TRY_CONVERT.readValue( VPackUtils.toVPack("{\"l\":-2.25 }"), LongWrapper.class);
            assertEquals(-2L, w.l);
            long[] l = MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("[ 2.2 ]"), long[].class);
            assertEquals(1, l.length);
            assertEquals(2L, l[0]);
        }

        assertEquals(Short.valueOf((short)1), MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("1.2"), Short.class));
        assertEquals(Short.valueOf((short) 19), MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("19.2"), Short.TYPE));

        assertEquals(Byte.valueOf((byte)1), MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("1.2"), Byte.class));
        assertEquals(Byte.valueOf((byte) 1), MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("1.5"), Byte.TYPE));

        assertEquals(BigInteger.valueOf(124L), MAPPER_TRY_CONVERT.readValue(VPackUtils.toVPack("124.2"), BigInteger.class));
    }
