...
public class StreamSerializerTest extends StreamTestBase
{
    ...
    {
        ObjectMapper mapper = DatabindTestUtil.vpackMapperBuilder()
                .enable(MapperFeature.USE_STATIC_TYPING)
                .build();
        String json = VPackUtils.toJson(mapper.writeValueAsBytes(
    ...
    public void testStreamOfStrings() throws Exception
    {
        assertEquals("[\"hello\",\"world\"]",
                VPackUtils.toJson(objectMapper.writeValueAsBytes(Stream.of("hello", "world"))));
    }

    @Test
    public void testStreamOfIntegers() throws Exception
    {
        assertEquals("[1,2,3]", VPackUtils.toJson(objectMapper.writeValueAsBytes(Stream.of(1, 2, 3))));
    }

    private <T, R> R[] roundTrip(Stream<T> stream, Class<R[]> clazz) {
        String json = VPackUtils.toJson(objectMapper.writeValueAsBytes(stream));
        return objectMapper.readValue(VPackUtils.toVPack(json), clazz);
    }
