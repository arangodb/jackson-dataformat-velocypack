    ...
    extends DatabindTestUtil
{
    ...
    public void testBeanWithCreator() throws Exception
    {
        ...
        final String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(emptyEntity));

        final Issue4Entity deserialisedEntity = MAPPER.readValue(VPackUtils.toVPack(json), Issue4Entity.class);
    ...
    {
        OptionalStringBean bean = MAPPER.readValue(VPackUtils.toVPack("{\"value\":\"xyz\"}"), OptionalStringBean.class);
    ...
    public void testExcludeIfOptionalAbsent() throws Exception
    {
        ...
        assertEquals(a2q("{'value':'foo'}"),
                VPackUtils.toJson(mapper.writeValueAsBytes(new OptionalStringBean("foo"))));
        // absent is not strictly null so
        assertEquals(a2q("{'value':null}"),
                VPackUtils.toJson(mapper.writeValueAsBytes(new OptionalStringBean(null))));
    ...
    {
        CaseChangingStringWrapper w = MAPPER.readValue(VPackUtils.toVPack(a2q("{'value':'FoobaR'}")),
    ...
    public void testWithCustomDeserializerIfOptionalAbsent() throws Exception
    {
        ...
        //    specific handling
        /*
        assertEquals(Optional.empty(), MAPPER.readValue(VPackUtils.toVPack("{}"),
                CaseChangingStringWrapper.class).value);
                */

        assertEquals(Optional.empty(), MAPPER.readValue(VPackUtils.toVPack(a2q("{'value':null}")),
                CaseChangingStringWrapper.class).value);
    }
    ...
    public void testCustomSerializer() throws Exception
    {
        ...
        String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(new CaseChangingStringWrapper(VALUE)));
    ...
    public void testCustomSerializerIfOptionalAbsent() throws Exception
    {
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        assertEquals(a2q("{'value':'FOO'}"),
                VPackUtils.toJson(mapper.writeValueAsBytes(new CaseChangingStringWrapper("foo"))));
        // absent is not strictly null so
        assertEquals(a2q("{'value':null}"),
                VPackUtils.toJson(mapper.writeValueAsBytes(new CaseChangingStringWrapper(null))));
    ...
    {
        String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(new AtomicReference<String>("foo")));
