	...
	extends DateTimeTestBase
{
    ...
    public void testSerializationWithTypeInfo02() throws Exception
    {
        ...
        String json = VPackUtils.toJson(MAPPER.writer()
                .with(DateTimeFeature.WRITE_DATES_AS_TIMESTAMPS)
                .writeValueAsBytes(new Holder46(localDate, localDate)));
    ...
    public void testConfigOverrides() throws Exception
    {
        ...
        assertEquals(EXP_DATE, VPackUtils.toJson(mapper.writeValueAsBytes(date)));

        // and read back, too
        VanillaWrapper output = mapper.readValue(VPackUtils.toVPack(json), VanillaWrapper.class);
        assertEquals(input.value, output.value);
        LocalDate date2 = mapper.readValue(VPackUtils.toVPack(EXP_DATE), LocalDate.class);
    ...
    {
        EpochDayWrapper w = MAPPER.readValue(VPackUtils.toVPack("{\"value\": 1000}"), EpochDayWrapper.class);
