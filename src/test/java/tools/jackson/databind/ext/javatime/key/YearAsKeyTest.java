...
public class YearAsKeyTest extends DateTimeTestBase
{
    ...
    public void testKeySerialization() throws Exception {
        assertEquals(mapAsString("3141", "test"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(asMap(Year.of(3141), "test"))),
                "Value is incorrect");
    }

    @Test
    public void testKeyDeserialization() throws Exception {
        assertEquals(asMap(Year.of(3141), "test"), READER.readValue(VPackUtils.toVPack(mapAsString("3141", "test"))),
                "Value is incorrect");
        // Test both padded, unpadded
        assertEquals(asMap(Year.of(476), "test"), READER.readValue(VPackUtils.toVPack(mapAsString("0476", "test"))),
                "Value is incorrect");
        assertEquals(asMap(Year.of(476), "test"), READER.readValue(VPackUtils.toVPack(mapAsString("476", "test"))),
                "Value is incorrect");
    }
    ...
    public void deserializeYearKey_notANumber() throws Exception {
        assertThrows(InvalidFormatException.class, () -> {
            READER.readValue(VPackUtils.toVPack(mapAsString("10000BC", "test")));
        });
    }
    ...
    public void deserializeYearKey_notAYear() throws Exception {
        assertThrows(InvalidFormatException.class, () -> {
            READER.readValue(VPackUtils.toVPack(mapAsString(Integer.toString(Year.MAX_VALUE+1), "test")));
        });
    }
    ...
    public void serializeAndDeserializeYearKeyUnpadded() throws Exception {
        ...
        String serialized = VPackUtils.toJson(MAPPER.writeValueAsBytes(testMap));
        TypeReference<Map<Year, Float>> yearFloatTypeReference = new TypeReference<Map<Year, Float>>() {};
        Map<Year, Float> deserialized = MAPPER.readValue(VPackUtils.toVPack(serialized), yearFloatTypeReference);
        assertEquals(testMap, deserialized);

        // actually, check padded as well just to make sure
        Map<Year, Float> deserialized2 = MAPPER.readValue(VPackUtils.toVPack(a2q("{'0001':1.0}")),
