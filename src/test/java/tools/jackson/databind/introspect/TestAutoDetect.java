...
public class TestAutoDetect extends DatabindTestUtil
{
    ...
    public void testProtectedDelegatingCtor() throws Exception
    {
        ...
        ProtectedBean bean = MAPPER.readValue(VPackUtils.toVPack(q("abc")), ProtectedBean.class);
        assertEquals("abc", bean._a);

        // then by increasing visibility requirement:
        ObjectMapper m = vpackMapperBuilder()
                .changeDefaultVisibility(vc -> vc.withScalarConstructorVisibility(Visibility.PUBLIC_ONLY))
                .build();
        JacksonException e = assertThrows(JacksonException.class,
                () -> m.readValue(VPackUtils.toVPack("\"abc\""), ProtectedBean.class));
    ...
    {
        // first, default settings, with which construction works ok
        PrivateBeanAnnotated bean = MAPPER.readValue(VPackUtils.toVPack(q("abc")), PrivateBeanAnnotated.class);
        assertEquals("abc", bean.a);

        // but not so much without
        JacksonException e = assertThrows(JacksonException.class,
                () -> MAPPER.readValue(VPackUtils.toVPack("\"abc\""), PrivateBeanNonAnnotated.class));
        verifyException(e, InvalidDefinitionException.class, "no String-argument constructor/factory");

        // except if we lower requirement
        ObjectMapper m = vpackMapperBuilder()
                .changeDefaultVisibility(vc -> vc.withScalarConstructorVisibility(Visibility.ANY))
                .build();
        bean = m.readValue(VPackUtils.toVPack(q("xyz")), PrivateBeanAnnotated.class);
    ...
    public void testVisibilityConfigOverridesForDeser() throws Exception
    {
        ...
        DatabindException e = assertThrows(DatabindException.class,
                () -> MAPPER.readValue(VPackUtils.toVPack(JSON), Feature1347DeserBean.class));
        ...
        ObjectMapper mapper = vpackMapperBuilder()
                .withConfigOverride(Feature1347DeserBean.class,
                        o -> o.setVisibility(JsonAutoDetect.Value.construct(PropertyAccessor.SETTER,
                                Visibility.NONE)))
                .build();
        Feature1347DeserBean result = mapper.readValue(VPackUtils.toVPack(JSON), Feature1347DeserBean.class);
    ...
    public void testAnnotatedFieldIssue2789() throws Exception {
        final String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(new DataClassA()));
        final DataParent2789 copy = MAPPER.readValue(VPackUtils.toVPack(json), DataParent2789.class);
