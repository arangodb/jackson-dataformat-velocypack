...
public class AnnotationBundlesTest extends DatabindTestUtil
{
    ...
    public void testKeepAnnotationBundle() throws Exception
    {
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        assertEquals("{\"important\":42}", VPackUtils.toJson(mapper.writeValueAsBytes(new InformingHolder())));
    }

    @Test
    public void testRecursiveBundlesField() throws Exception {
        assertEquals("{\"unimportant\":42}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new RecursiveHolder())));
    }

    @Test
    public void testRecursiveBundlesMethod() throws Exception {
        assertEquals("{\"value\":28}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new RecursiveHolder2())));
    }

    @Test
    public void testRecursiveBundlesConstructor() throws Exception {
        RecursiveHolder3 result = MAPPER.readValue(VPackUtils.toVPack("17"), RecursiveHolder3.class);
    ...
    {
        assertEquals("{\"foobar\":13}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new Bean())));
    }

    @Test
    public void testVisibilityBundle() throws Exception
    {
        assertEquals("{\"b\":5}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new NoAutoDetect())));
    }

    @Test
    public void testIssue92() throws Exception
    {
        assertEquals("{\"_id\":\"abc\"}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new Bean92())));
    }
}
