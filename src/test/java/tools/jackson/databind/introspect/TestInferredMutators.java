...
public class TestInferredMutators extends DatabindTestUtil
{
    ...
    public void testFinalFieldIgnoral() throws Exception
    {
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        try {
            /*p =*/ mapper.readValue(VPackUtils.toVPack("{\"x\":2}"), FixedPoint.class);
    ...
    public void testDeserializationInference() throws Exception
    {
        ...
        ObjectMapper mapper = vpackMapperBuilder().enable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES).build();
        // First: default case, inference enabled:
        assertTrue(mapper.isEnabled(MapperFeature.INFER_PROPERTY_MUTATORS));
        Point p = mapper.readValue(VPackUtils.toVPack(JSON),  Point.class);
        assertEquals(2, p.x);
        ...
        mapper = vpackMapperBuilder()
                ...
                .build();
        try {
            p = mapper.readValue(VPackUtils.toVPack(JSON),  Point.class);
