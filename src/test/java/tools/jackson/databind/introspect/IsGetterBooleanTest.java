...
public class IsGetterBooleanTest extends DatabindTestUtil
{
    ...
    public void testAllowIntIsGetter() throws Exception
    {
        ...
        Map<?, ?> props = MAPPER.readValue(VPackUtils.toVPack(json), Map.class);
        assertEquals(Collections.singletonMap("enabled", 12),
                props);

        POJO3609 output = MAPPER.readValue(VPackUtils.toVPack(json), POJO3609.class);
    ...
    public void testBooleanReference() throws Exception
    {
        assertEquals(a2q("{'atomic':true}"),
                VPackUtils.toJson(sharedMapper().writeValueAsBytes(new POJO3836_AR())));
    }
    ...
    {
        assertEquals(a2q("{'atomic':true}"),
                VPackUtils.toJson(sharedMapper().writeValueAsBytes(new POJO3836_AB())));
    }
    ...
    {
        assertEquals(a2q("{'atomic':true}"),
                VPackUtils.toJson(sharedMapper().writeValueAsBytes(new POJO3836_OB())));
    }
    ...
    public void testIsPropertiesStdKotlin() throws Exception
    {
        ...
        POJO2527 output = mapper.readValue(VPackUtils.toVPack(json), POJO2527.class);
        assertEquals(input.isEnabled, output.isEnabled);
    }
    ...
    public void testIsPropertiesWithPublicField() throws Exception
    {
        ...
        POJO2527PublicField output = mapper.readValue(VPackUtils.toVPack(json), POJO2527PublicField.class);
        assertEquals(input.isEnabled, output.isEnabled);
    }
    ...
    public void testIsPropertiesViaCreator() throws Exception
    {
        ...
        POJO2527Creator output = mapper.readValue(VPackUtils.toVPack(json), POJO2527Creator.class);
        assertEquals(input.isEnabled, output.isEnabled);
    }
}
