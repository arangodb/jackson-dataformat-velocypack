...
//        @JsonDeserialize
    ...
    public void testSimpleGetters() throws Exception
    {
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        assertEquals("{\"Get-key\":123}", VPackUtils.toJson(mapper.writeValueAsBytes(new GetterBean())));
    }
    ...
    public void testSimpleSetters() throws Exception
    {
        ObjectMapper mapper = vpackMapperBuilder()
                .propertyNamingStrategy(new PrefixStrategy())
                .build();
        SetterBean bean = mapper.readValue(VPackUtils.toVPack("{\"Set-key\":13}"), SetterBean.class);
    ...
    public void testSimpleFields() throws Exception
    {
        ...
        ObjectMapper mapper = vpackMapperBuilder()
                .propertyNamingStrategy(new PrefixStrategy())
                .build();
        String json = VPackUtils.toJson(mapper.writeValueAsBytes(new FieldBean(999)));
        assertEquals("{\"Field-key\":999}", json);

        // then deserialize
        FieldBean result = mapper.readValue(VPackUtils.toVPack(json), FieldBean.class);
    ...
    public void testCStyleNaming() throws Exception
    {
        ...
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        String json = VPackUtils.toJson(mapper.writeValueAsBytes(new PersonBean("Joe", "Sixpack", 42)));
        assertEquals("{\"first_name\":\"Joe\",\"last_name\":\"Sixpack\",\"age\":42}", json);

        // then deserialize
        PersonBean result = mapper.readValue(VPackUtils.toVPack(json), PersonBean.class);
    ...
    public void testWithGetterAsSetter() throws Exception
    {
        ...
        SetterlessWithValue result = mapper.readValue(VPackUtils.toVPack(json), SetterlessWithValue.class);
...
//        mapper.disable(DeserializationConfig.DeserializationFeature.USE_GETTERS_AS_SETTERS);
        RenamedCollectionBean result = mapper.readValue(VPackUtils.toVPack("{\"thevalues\":[\"a\"]}"),
    ...
    public void testPerClassAnnotation() throws Exception
    {
        ...
        BeanWithPrefixNames output = mapper.readValue(VPackUtils.toVPack("{\"Set-a\":7}"),
