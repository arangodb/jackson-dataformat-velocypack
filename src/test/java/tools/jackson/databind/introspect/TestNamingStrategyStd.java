...
public class TestNamingStrategyStd extends DatabindTestUtil
{
    ...
    private final static ObjectMapper _lcWithUnderscoreMapper = vpackMapperBuilder()
        .propertyNamingStrategy(PropertyNamingStrategies.SNAKE_CASE)
        .build();

    private final static ObjectMapper _ucWithUnderscoreMapper = vpackMapperBuilder()
    ...
    {
        // First serialize
        String json = VPackUtils.toJson(_lcWithUnderscoreMapper.writeValueAsBytes(new PersonBean("Joe", "Sixpack", 42)));
        assertEquals("{\"first_name\":\"Joe\",\"last_name\":\"Sixpack\",\"age\":42}", json);

        // then deserialize
        PersonBean result = _lcWithUnderscoreMapper.readValue(VPackUtils.toVPack(json), PersonBean.class);
    ...
    {
        // First serialize
        String json = VPackUtils.toJson(_lcWithUnderscoreMapper.writer()
                .writeValueAsBytes(new Acronyms("world wide web", "http://jackson.codehaus.org", "/path1/,/path2/")));
        assertEquals("{\"www\":\"world wide web\",\"some_url\":\"http://jackson.codehaus.org\",\"some_uris\":\"/path1/,/path2/\"}", json);

        // then deserialize
        Acronyms result = _lcWithUnderscoreMapper.readValue(VPackUtils.toVPack(json), Acronyms.class);
    ...
    {
        // First serialize
        String json = VPackUtils.toJson(_lcWithUnderscoreMapper.writeValueAsBytes(new OtherNonStandardNames("Results", "_User", "___", "$User")));
        assertEquals("{\"results\":\"Results\",\"user\":\"_User\",\"__\":\"___\",\"$_user\":\"$User\"}", json);

        // then deserialize
        OtherNonStandardNames result = _lcWithUnderscoreMapper.readValue(VPackUtils.toVPack(json), OtherNonStandardNames.class);
    ...
    {
        // First serialize
        String json = VPackUtils.toJson(_lcWithUnderscoreMapper.writeValueAsBytes(new UnchangedNames("from_user", "_user", "from$user", "from7user", "_x")));
        assertEquals("{\"from_user\":\"from_user\",\"user\":\"_user\",\"from$user\":\"from$user\",\"from7user\":\"from7user\",\"x\":\"_x\"}", json);

        // then deserialize
        UnchangedNames result = _lcWithUnderscoreMapper.readValue(VPackUtils.toVPack(json), UnchangedNames.class);
...
"      }\n" +
"    }";
        Value3368 res = sharedMapper().readerFor(Value3368.class).readValue(VPackUtils.toVPack(test));
    ...
    {
        // First serialize
        String json = VPackUtils.toJson(_ucWithUnderscoreMapper
            .writeValueAsBytes(new PersonBean("Joe", "Sixpack", 42)));
        assertEquals("{\"FIRST_NAME\":\"Joe\",\"LAST_NAME\":\"Sixpack\",\"AGE\":42}", json);

        // then deserialize
        PersonBean result = _ucWithUnderscoreMapper.readValue(VPackUtils.toVPack(json), PersonBean.class);
        assertEquals("Joe", result.firstName);
        assertEquals("Sixpack", result.lastName);
        assertEquals(42, result.age);
    }
    ...
    public void testSimpleLowerCase() throws Exception
    {
        ...
        assertEquals(a2q("{'firstname':'Bob','lastname':'Burger'}"),
                VPackUtils.toJson(m.writeValueAsBytes(input)));
    }
    ...
    public void testSimpleKebabCase() throws Exception
    {
        ...
        ObjectMapper m = vpackMapperBuilder()
                ...
                .build();
        assertEquals(a2q("{'first-name':'Bob'}"), VPackUtils.toJson(m.writeValueAsBytes(input)));

        FirstNameBean result = m.readValue(VPackUtils.toVPack(a2q("{'first-name':'Billy'}")),
                FirstNameBean.class);
    ...
    public void testNamingWithObjectNode() throws Exception
    {
        ...
        ClassWithObjectNodeField result =
            m.readValue(
                VPackUtils.toVPack("{ \"id\": \"1\", \"json\": { \"foo\": \"bar\", \"baz\": \"bing\" } }"),
    ...
    public void testExplicitRename() throws Exception
    {
        ...
        assertEquals(a2q("{'firstName':'Peter','lastName':'Venkman','user_age':'35'}"),
                VPackUtils.toJson(m.writeValueAsBytes(new ExplicitBean())));
        ...
        ExplicitBean bean =
                m.readValue(VPackUtils.toVPack(a2q("{'first_name':'Egon','last_name':'Spengler','user_age':'32'}")),
                        ExplicitBean.class);
    ...
    public void testExplicitNoNaming() throws Exception
    {
        ...
        String json = VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultNaming()));
    ...
    public void testNamingViaConstructorParams() throws Exception
    {
        ...
        SnakeNameBean value = mapper.readValue(
                VPackUtils.toVPack(a2q("{'id':'foobar', 'full_name' : 'Foo Bar'}")),
                SnakeNameBean.class);
