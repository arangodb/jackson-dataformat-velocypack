...
public class MapEntryFormatTest extends DatabindTestUtil
{
    ...
    public void testInclusion() throws Exception
    {
        assertEquals(a2q("{'entry':{'a':'b'}}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new EmptyEntryWrapper("a", "b"))));
        assertEquals(a2q("{'entry':{'a':'b'}}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new EntryWithDefaultWrapper("a", "b"))));
        assertEquals(a2q("{'entry':{'a':'b'}}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new EntryWithNullWrapper("a", "b"))));

        assertEquals(a2q("{}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new EmptyEntryWrapper("a", ""))));
        assertEquals(a2q("{}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new EntryWithDefaultWrapper("a", ""))));
        assertEquals(a2q("{'entry':{'a':''}}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new EntryWithNullWrapper("a", ""))));
        assertEquals(a2q("{}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new EntryWithNullWrapper("a", null))));
    }
    ...
    {
        assertEquals(a2q("{'entry':{'a':'b'}}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new EntryWithNonAbsentWrapper("a", "b"))));
        // empty String not excluded since reference is not absent, just points to empty
        // (so would need 3rd level inclusion definition)
        assertEquals(a2q("{'entry':{'a':''}}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new EntryWithNonAbsentWrapper("a", ""))));
        assertEquals(a2q("{}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new EntryWithNonAbsentWrapper("a", null))));
    }
    ...
    public void testAsNaturalRoundtrip() throws Exception
    {
        ...
        BeanWithMapEntry result = MAPPER.readValue(VPackUtils.toVPack(json), BeanWithMapEntry.class);
    ...
    public void testAsObjectRoundtrip() throws Exception
    {
        ...
        MapEntryAsObject result = MAPPER.readValue(VPackUtils.toVPack(json), MapEntryAsObject.class);
    ...
    public void testDefaultShapeOverride() throws Exception
    {
        ...
        assertTrue(VPackUtils.toJson(mapper.writeValueAsBytes(input)).equals(a2q("{'key':'foo','value':'bar'}"))
                || VPackUtils.toJson(mapper.writeValueAsBytes(input)).equals(a2q("{'value':'bar','key':'foo'}")));
    }
    ...
    public void testWrappedAsObjectRoundtrip1419() throws Exception
    {
        ...
        BeanWithMapEntryAsPOJO result = MAPPER.readValue(VPackUtils.toVPack(json), BeanWithMapEntryAsPOJO.class);
        assertEquals("foo", result.entry.getKey());
        assertEquals("bar", result.entry.getValue());
    }
    ...
    public void testWrappedAsComplexRoundtrip1419() throws Exception
    {
        ...
        BeanWithComplexMapEntryAsPOJO result = MAPPER.readValue(VPackUtils.toVPack(json),
    ...
    public void testDeserFailWithStructureMismatch1419() throws Exception
    {
        ...
        UnrecognizedPropertyException e = assertThrows(UnrecognizedPropertyException.class,
                () -> strictMapper.readValue(VPackUtils.toVPack(a2q("{'entry':{'notKey': 'value'}}")),
