...
public class StrictVPackTypeInfoHandling3853Test extends DatabindTestUtil {
    ...
    public void testDefaultHasStrictTypeHandling() throws Exception {
        ObjectMapper om = vpackMapperBuilder()
    ...
    public void testMissingTypeId() throws Exception {
        final ObjectMapper ENABLED_MAPPER = vpackMapperBuilder().enable(MapperFeature.REQUIRE_TYPE_ID_FOR_SUBTYPES).build();
        final ObjectMapper DISABLED_MAPPER = vpackMapperBuilder().disable(MapperFeature.REQUIRE_TYPE_ID_FOR_SUBTYPES).build();
        final ObjectMapper DEFAULT_MAPPER = vpackMapperBuilder().build();
    ...
    private void verifyInvalidTypeIdWithSuperclassTarget(ObjectMapper om) throws Exception {
        InvalidTypeIdException e = assertThrows(InvalidTypeIdException.class,
                () -> om.readValue(VPackUtils.toVPack("{}"), Command.class));
    ...
    private void verifyInvalidTypeIdWithConcreteTarget(ObjectMapper om) throws Exception {
        InvalidTypeIdException e = assertThrows(InvalidTypeIdException.class,
                () -> om.readValue(VPackUtils.toVPack("{}"), DoSomethingCommand.class));
        verifyException(e, "missing type id property '@type'");
    }

    private void verifyDeserializationWithConcreteTarget(ObjectMapper om) throws Exception {
        DoSomethingCommand cmd = om.readValue(VPackUtils.toVPack("{}"), DoSomethingCommand.class);
    ...
    private void verifyDeserializationWithFullTypeInfo(ObjectMapper om) throws Exception {
        Command cmd = om.readValue(VPackUtils.toVPack("{\"@type\":\"do-something\"}"), Command.class);
        assertInstanceOf(DoSomethingCommand.class, cmd);
        cmd = om.readValue(VPackUtils.toVPack("{\"@type\":\"do-something\"}"), DoSomethingCommand.class);
        assertInstanceOf(DoSomethingCommand.class, cmd);
    }

    private <T> void verifySuccessWithNonNullAndType(String json, Class<T> clazz, ObjectMapper om) throws Exception {
        T bean = om.readValue(VPackUtils.toVPack(json), clazz);
        assertNotNull(bean);
    ...
    private void verifyFailureMissingTypeId(String json, Class<?> clazz, ObjectMapper om) throws Exception {
        InvalidTypeIdException e = assertThrows(InvalidTypeIdException.class,
                () -> om.readValue(VPackUtils.toVPack(json), clazz));
        verifyException(e, "missing type id property '@type'");
    }
}
