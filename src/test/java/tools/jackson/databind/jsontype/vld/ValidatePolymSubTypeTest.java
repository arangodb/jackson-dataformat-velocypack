...
public class ValidatePolymSubTypeTest extends DatabindTestUtil
{
    ...
    private final ObjectMapper MAPPER_DEF_TYPING_NAME_CHECK = vpackMapperBuilder()
            .activateDefaultTyping(new SimpleNameBasedValidator())
            .build();

    private final ObjectMapper MAPPER_DEF_TYPING_CLASS_CHECK = vpackMapperBuilder()
            .activateDefaultTyping(new SimpleNameBasedValidator())
            .build();

    // // // Mappers without Default Typing (explicit annotation needed)

    private final ObjectMapper MAPPER_EXPLICIT_NAME_CHECK = vpackMapperBuilder()
            .polymorphicTypeValidator(new SimpleNameBasedValidator())
            .build();

    private final ObjectMapper MAPPER_EXPLICIT_CLASS_CHECK = vpackMapperBuilder()
    ...
    private DefTypeWrapper _roundTripDefault(ObjectMapper mapper, BaseValue input) throws Exception {
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new DefTypeWrapper(input)));
        return mapper.readValue(VPackUtils.toVPack(json), DefTypeWrapper.class);
    }

    private AnnotatedWrapper _roundTripAnnotated(ObjectMapper mapper, BaseValue input) throws Exception {
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new AnnotatedWrapper(input)));
        return mapper.readValue(VPackUtils.toVPack(json), AnnotatedWrapper.class);
    }

    private AnnotatedMinimalWrapper _roundTripAnnotatedMinimal(ObjectMapper mapper, BaseValue input) throws Exception {
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new AnnotatedMinimalWrapper(input)));
        return mapper.readValue(VPackUtils.toVPack(json), AnnotatedMinimalWrapper.class);
    }
    ...
    private void _verifyBadDefaultValue(ObjectMapper mapper) throws Exception {
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new DefTypeWrapper(new BadValue())));
        _verifyBadValue(mapper, json, DefTypeWrapper.class);
    }

    private void _verifyMehDefaultValue(ObjectMapper mapper) throws Exception {
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new DefTypeWrapper(new MehValue())));
    ...
    private void _verifyBadAnnotatedValue(ObjectMapper mapper) throws Exception {
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new AnnotatedWrapper(new BadValue())));
    ...
    private void _verifyMehAnnotatedValue(ObjectMapper mapper) throws Exception {
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new AnnotatedWrapper(new MehValue())));
    ...
    private void _verifyBadAnnotatedMinValue(ObjectMapper mapper) throws Exception {
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new AnnotatedMinimalWrapper(new BadValue())));
    ...
    private void _verifyMehAnnotatedMinValue(ObjectMapper mapper) throws Exception {
        final String json = VPackUtils.toJson(mapper.writeValueAsBytes(new AnnotatedMinimalWrapper(new MehValue())));
