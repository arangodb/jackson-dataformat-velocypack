...
public class BasicPTVWithArraysTest extends DatabindTestUtil
{
    ...
    public void testAllowBySubClassInArray() throws Exception {
        PolymorphicTypeValidator ptv = BasicPolymorphicTypeValidator.builder()
                ...
                .build();
        final ObjectMapper mapper1 = vpackMapperBuilder()
                .activateDefaultTyping(ptv, DefaultTyping.NON_FINAL)
                .build();

        final String json = VPackUtils.toJson(mapper1.writeValueAsBytes(new ObjectWrapper(new Base2534[] { new Good2534(42) })));

        // First test blocked case:
        InvalidTypeIdException e = assertThrows(InvalidTypeIdException.class,
                () -> mapper1.readValue(VPackUtils.toVPack(json), ObjectWrapper.class));
        ...
        PolymorphicTypeValidator ptv2 = BasicPolymorphicTypeValidator.builder()
                ...
                .allowIfSubType(Good2534.class)
                .build();
        final ObjectMapper mapper2 = vpackMapperBuilder()
                .activateDefaultTyping(ptv2, DefaultTyping.NON_FINAL)
                .build();

        ObjectWrapper w = mapper2.readValue(VPackUtils.toVPack(json), ObjectWrapper.class);
        assertNotNull(w);
        ...
        final String badJson = VPackUtils.toJson(mapper2.writeValueAsBytes(new ObjectWrapper(new Base2534[] { new Bad2534(13) })));
        InvalidTypeIdException e2 = assertThrows(InvalidTypeIdException.class,
                () -> mapper2.readValue(VPackUtils.toVPack(badJson), ObjectWrapper.class));
