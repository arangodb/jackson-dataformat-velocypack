...
public class CustomPTVMatchersTest extends DatabindTestUtil
{
    ...
    public void testCustomBaseMatchers() throws Exception
    {
        ...
        // runtime type here)
        final String goodJson = VPackUtils.toJson(mapper.writeValueAsBytes(new CustomBad(42) ));
        CustomBase result = mapper.readValue(VPackUtils.toVPack(goodJson), CustomBase.class);
        ...
        final String badJson = VPackUtils.toJson(mapper.writeValueAsBytes(TimeZone.getDefault()));
        InvalidTypeIdException e = assertThrows(InvalidTypeIdException.class,
                () -> mapper.readValue(VPackUtils.toVPack(badJson), TimeZone.class));
    ...
    public void testCustomSubtypeMatchers() throws Exception
    {
        ...
        final String goodJson = VPackUtils.toJson(mapper.writeValueAsBytes(new ObjectWrapper(new CustomGood(42) )));
        ObjectWrapper w = mapper.readValue(VPackUtils.toVPack(goodJson), ObjectWrapper.class);
        assertNotNull(w);
        assertNotNull(w.value);
        assertEquals(CustomGood.class, w.value.getClass());

        // Then something else
        final String badJson = VPackUtils.toJson(mapper.writeValueAsBytes(new ObjectWrapper(new CustomBad(42) )));
        InvalidTypeIdException e = assertThrows(InvalidTypeIdException.class,
                () -> mapper.readValue(VPackUtils.toVPack(badJson), ObjectWrapper.class));
