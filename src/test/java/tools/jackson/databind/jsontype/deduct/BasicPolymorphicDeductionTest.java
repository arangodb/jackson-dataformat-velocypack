...
public class BasicPolymorphicDeductionTest extends DatabindTestUtil {
  ...
  public void testSimpleInference() throws Exception {
    Cat cat = MAPPER.readValue(VPackUtils.toVPack(liveCatJson), Cat.class);
    ...
    cat = MAPPER.readValue(VPackUtils.toVPack(deadCatJson), Cat.class);
  ...
  public void testSimpleInferenceOfEmptySubtype() throws Exception {
    ...
    // When:
    Feline feline = mapper.readValue(VPackUtils.toVPack(fleabagJson), Feline.class);
    // Then:
  ...
  public void testSimpleInferenceOfEmptySubtypeDoesntMatchNull() throws Exception {
    ...
    ObjectMapper mapper = MAPPER;
    // When:
    Feline feline = mapper.readValue(VPackUtils.toVPack("null"), Feline.class);
    // Then:
    assertNull(feline);
  }
  ...
  public void testCaseInsensitiveInference() throws Exception {
    Cat cat = vpackMapperBuilder() // Don't use shared mapper!
      .configure(MapperFeature.ACCEPT_CASE_INSENSITIVE_PROPERTIES, true)
      .build()
      .readValue(VPackUtils.toVPack(deadCatJson.toUpperCase()), Cat.class);
    assertInstanceOf(DeadCat.class, cat);
    assertSame(cat.getClass(), DeadCat.class);
    assertEquals("FELIX", cat.name);
    assertEquals("ENTROPY", ((DeadCat)cat).causeOfDeath);
...
//    ObjectMapper mapper = VPackMapper.builder() // Don't use shared mapper!
...
//    Cat cat = mapper.readValue(VPackUtils.toVPack(deadCatJson.replace("causeOfDeath", "CAUSEOFDEATH")), Cat.class);
//    assertInstanceOf(DeadCat.class, cat);
//    assertSame(cat.getClass(), DeadCat.class);
//    assertEquals("Felix", cat.name);
//    assertEquals("Entropy", ((DeadCat)cat).causeOfDeath);
//  }
  ...
  public void testContainedInference() throws Exception {
    Box box = MAPPER.readValue(VPackUtils.toVPack(box1Json), Box.class);
    ...
    box = MAPPER.readValue(VPackUtils.toVPack(box2Json), Box.class);
  ...
  public void testContainedInferenceOfEmptySubtype() throws Exception {
    Box box = MAPPER.readValue(VPackUtils.toVPack(box3Json), Box.class);
    assertInstanceOf(Fleabag.class, box.feline);

    box = MAPPER.readValue(VPackUtils.toVPack(box4Json), Box.class);
    assertNull(box.feline, "null != {}");

    box = MAPPER.readValue(VPackUtils.toVPack(box5Json), Box.class);
  ...
  public void testListInference() throws Exception {
    ...
    List<Cat> boxes = MAPPER.readValue(VPackUtils.toVPack(arrayOfCatsJson), listOfCats);
  ...
  public void testMapInference() throws Exception {
    ...
    Map<String, Cat> map = MAPPER.readValue(VPackUtils.toVPack(mapOfCatsJson), mapOfCats);
  ...
  public void testArrayInference() throws Exception {
    Cat[] boxes = MAPPER.readValue(VPackUtils.toVPack(arrayOfCatsJson), Cat[].class);
  ...
  public void testIgnoreProperties() throws Exception {
    Cat cat = MAPPER.readerFor(Cat.class)
      ...
      .readValue(VPackUtils.toVPack(luckyCatJson));
    assertInstanceOf(LiveCat.class, cat);
    assertSame(cat.getClass(), LiveCat.class);
    assertEquals("Felix", cat.name);
    assertTrue(((LiveCat)cat).angry);
  ...
  public void testAmbiguousClasses() throws Exception {
    ObjectMapper mapper = vpackMapperBuilder() // Don't use shared mapper!
            .registerSubtypes(AnotherLiveCat.class)
            .build();
    InvalidDefinitionException e = assertThrows(InvalidDefinitionException.class,
            () -> mapper.readValue(VPackUtils.toVPack(liveCatJson), Cat.class));
  ...
  public void testAmbiguousProperties() throws Exception {
    InvalidTypeIdException e = assertThrows(InvalidTypeIdException.class,
            () -> MAPPER.readValue(VPackUtils.toVPack(ambiguousCatJson), Cat.class));
  ...
  public void testFailOnInvalidSubtype() throws Exception {
    ...
    ObjectMapper mapper = vpackMapperBuilder() // Don't use shared mapper!
      ...
      .build();
    // When:
    Cat cat = mapper.readValue(VPackUtils.toVPack(ambiguousCatJson), Cat.class);
    // Then:
    assertNull(cat);
  }
  ...
  public void testSimpleSerialization() throws Exception {
    // Given:
    JavaType listOfCats = defaultTypeFactory().constructParametricType(List.class, Cat.class);
    List<Cat> list = MAPPER.readValue(VPackUtils.toVPack(arrayOfCatsJson), listOfCats);
    Cat cat = list.get(0);
    // When:
    String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(cat));
    // Then:
  ...
  {
      assertEquals(q("value"), VPackUtils.toJson(MAPPER.writeValueAsBytes(new Bean3711())));
  }
  ...
  {
      assertEquals(q("B"), VPackUtils.toJson(MAPPER.writeValueAsBytes(Enum3711.B)));
  }
    ...
    public void testAliasWithPolymorphicDeduction4327(String field) throws Exception {
        ...
        Deduction4327 value = MAPPER.readValue(VPackUtils.toVPack(json), Deduction4327.class);
        assertNotNull(value);
