...
class ParsingContextExtTypeId2747Test extends DatabindTestUtil {
    ...
    void locationAccessWithExtTypeId() throws Exception {
        ObjectReader objectReader = newVPackMapper().readerFor(Wrapper.class);

        // Scalar value payload
        Wrapper wrapper = objectReader.readValue(VPackUtils.toVPack("{" +
                "\"type\":\"location\"," +
                "\"wrapped\": 1" +
                "}"));
        ...
        wrapper = objectReader.readValue(VPackUtils.toVPack("{" +
                "\"type\":\"location\"," +
                "\"wrapped\": {}" +
                "}"));
        assertEquals("/wrapped", ((Location) wrapper.wrapped).value);
    ...
    void locationAccessWithExtTypeIdCreator() throws Exception {
        ObjectReader objectReader = newVPackMapper().readerFor(CreatorWrapper.class);

        CreatorWrapper w = objectReader.readValue(VPackUtils.toVPack("{" +
                "\"type\":\"location\"," +
                "\"wrapped\": 1" +
                "}"));
        assertEquals("/wrapped", ((Location) w.wrapped).value);

        w = objectReader.readValue(VPackUtils.toVPack("{" +
                "\"wrapped\": {}," +
                "\"type\":\"location\"" +
                "}"));
        assertEquals("/wrapped", ((Location) w.wrapped).value);
    ...
    void locationAccessWithExtTypeIdDefaultImpl() throws Exception {
        ObjectReader objectReader = newVPackMapper().readerFor(DefaultImplWrapper.class);

        DefaultImplWrapper w = objectReader.readValue(VPackUtils.toVPack("{\"wrapped\": {}}"));
        assertEquals("/wrapped", ((Location) w.wrapped).value);
    }
}
