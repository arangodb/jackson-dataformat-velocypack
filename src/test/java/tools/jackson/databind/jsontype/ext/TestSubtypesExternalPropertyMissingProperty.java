...
public class TestSubtypesExternalPropertyMissingProperty extends DatabindTestUtil
{
    ...
    private final ObjectReader READER = newVPackMapper().reader();
    ...
    private void checkOrangeBox(ObjectReader r) throws Exception {
        Box deserOrangeBox = r.forType(Box.class).readValue(VPackUtils.toVPack(orangeBoxJson));
    ...
    private void checkAppleBox(ObjectReader r) throws Exception {
        Box deserAppleBox = r.forType(Box.class).readValue(VPackUtils.toVPack(appleBoxJson));
    ...
    private void checkOrangeBoxEmpty(ObjectReader r, String json) throws Exception {
        Box deserOrangeBox = r.forType(Box.class).readValue(VPackUtils.toVPack(json));
        assertEquals(orangeBox.type, deserOrangeBox.type);
    ...
    private void checkAppleBoxEmpty(ObjectReader r, String json) throws Exception {
        Box deserAppleBox = r.forType(Box.class).readValue(VPackUtils.toVPack(json));
        assertEquals(appleBox.type, deserAppleBox.type);
    ...
    private void checkBoxDatabindException(ObjectReader r, String json) throws Exception {
        MismatchedInputException e = assertThrows(MismatchedInputException.class,
                () -> r.forType(Box.class).readValue(VPackUtils.toVPack(json)));
    ...
    private void checkReqBoxDatabindException(ObjectReader r, String json) throws Exception {
        MismatchedInputException e = assertThrows(MismatchedInputException.class,
                () -> r.forType(ReqBox.class).readValue(VPackUtils.toVPack(json)));
        verifyException(e, "Missing property 'fruit' for external type id 'type'");
    }
}
