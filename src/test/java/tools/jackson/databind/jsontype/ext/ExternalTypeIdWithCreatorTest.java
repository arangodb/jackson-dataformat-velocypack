...
public class ExternalTypeIdWithCreatorTest extends DatabindTestUtil
{
    ...
    public void testExternalTypeId() throws Exception
    {
        ...
        Message<?> msg = MAPPER.readValue(VPackUtils.toVPack(a2q("{ 'type':'foo', 'payload': {} }")), type);
        assertNotNull(msg);
        assertNotNull(msg.payload);
        assertEquals("foo", msg.type);

        // and then with different order
        msg = MAPPER.readValue(VPackUtils.toVPack(a2q("{'payload': {}, 'type':'foo' }")), type);
        assertNotNull(msg);
        assertNotNull(msg.payload);
        assertEquals("foo", msg.type);
    }
    ...
    public void test1198Fails() throws Exception {
        ...
        Character character = MAPPER.readValue(VPackUtils.toVPack(json), Character.class);
    ...
    public void testExternalTypeIdWithEnum1328() throws Exception {
        String json = VPackUtils.toJson(MAPPER.writer()
                .writeValueAsBytes(Arrays.asList(new AnimalAndType1328(AnimalType1328.Dog, new Dog1328()))));
    ...
    {
        ObjectMapper mapper3045 = vpackMapperBuilder()
    ...
    {
        MyJson3045 result = mapper.readValue(VPackUtils.toVPack(input), MyJson3045.class);
