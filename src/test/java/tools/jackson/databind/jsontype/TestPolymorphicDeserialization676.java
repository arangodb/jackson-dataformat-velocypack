...
public class TestPolymorphicDeserialization676 extends DatabindTestUtil
{
    ...
    public void testDeSerFail() throws IOException {
        ...
        assertEquals(originMap,
                mapper.readValue(VPackUtils.toVPack(VPackUtils.toJson(mapper.writeValueAsBytes(originMap))), MapContainer.class));
    }
    ...
    public void testDeSerCorrect() throws IOException {
        ...
        assertEquals(new MapContainer(map),
                mapper.readValue(VPackUtils.toVPack(VPackUtils.toJson(mapper.writeValueAsBytes(new MapContainer(map)))),
                        MapContainer.class));
        ...
        assertEquals(originMap, mapper.readValue(VPackUtils.toVPack(VPackUtils.toJson(mapper.writeValueAsBytes(originMap))), MapContainer.class));
    }
    ...
    private MapContainer createDeSerMapContainer(MapContainer src, ObjectMapper mapper) throws IOException {
        ...
        result.value = src;
        String json = VPackUtils.toJson(mapper.writeValueAsBytes(result));
        assertEquals("{\"value\":{\"@class\":"
                + "\""+getClass().getName()+"$MapContainer\","
                + "\"map\":{\"DateValue\":[\"java.util.Date\",123456]}}}",
                json);
        PolymorphicValueWrapper deserializedResult = mapper.readValue(VPackUtils.toVPack(json), PolymorphicValueWrapper.class);
