    ...
    extends DatabindTestUtil
{
    ...
    public void testDeductionWithDefaultTypeResolverBuilder() throws Exception {
        ...
        final Parent firstRead = mapper.readValue(VPackUtils.toVPack("{ \"one\": \"Hello World\" }"), Parent.class);
        assertNotNull(firstRead);
        assertInstanceOf(Parent.ChildOne.class, firstRead);
        assertEquals("Hello World", ((Parent.ChildOne) firstRead).one);
        final Parent secondRead = mapper.readValue(VPackUtils.toVPack("{ \"two\": \"Hello World\" }"), Parent.class);
