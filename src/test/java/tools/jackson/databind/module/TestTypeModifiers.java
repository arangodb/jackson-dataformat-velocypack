...
public class TestTypeModifiers extends DatabindTestUtil
{
    ...
    private final ObjectMapper MY_TYPE_MAPPER = vpackMapperBuilder()
            .typeFactory(defaultTypeFactory().withModifier(new MyTypeModifier()))
            .build();

    private final ObjectMapper MAPPER_WITH_MODIFIER = vpackMapperBuilder()
            .typeFactory(defaultTypeFactory().withModifier(new MyTypeModifier()))
    ...
    public void testTypeResolutionForRecursive() throws Exception
    {
        final ObjectMapper mapper = vpackMapperBuilder()
                .typeFactory(defaultTypeFactory().withModifier(new MyTypeModifier()))
                .build();
        assertNotNull(mapper.readTree(VPackUtils.toVPack("{}")));
    }
    ...
    {
        assertEquals("[19]", VPackUtils.toJson(MAPPER_WITH_MODIFIER.writeValueAsBytes(new MyCollectionLikeType(19))));
    }
    ...
    public void testMapLikeSerialization() throws Exception
    {
        ...
        assertEquals("{\"x\":\"xxx:3\"}", VPackUtils.toJson(MAPPER_WITH_MODIFIER.writeValueAsBytes(new MyMapLikeType("x", 3))));
    }

    @Test
    public void testCollectionLikeDeserialization() throws Exception
    {
        MyMapLikeType result = MAPPER_WITH_MODIFIER.readValue(VPackUtils.toVPack("{\"a\":13}"), MyMapLikeType.class);
    ...
    {
        MyCollectionLikeType result = MAPPER_WITH_MODIFIER.readValue(VPackUtils.toVPack("[-37]"), MyCollectionLikeType.class);
