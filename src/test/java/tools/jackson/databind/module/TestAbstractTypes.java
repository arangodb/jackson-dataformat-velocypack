...
public class TestAbstractTypes extends DatabindTestUtil
{
    ...
    public void testCollectionDefaulting() throws Exception
    {
        ...
        ObjectMapper mapper = vpackMapperBuilder()
                .addModule(mod)
                .build();
        Collection<?> result = mapper.readValue(VPackUtils.toVPack("[]"), Collection.class);
    ...
    public void testMapDefaultingBasic() throws Exception
    {
        ...
        ObjectMapper mapper = vpackMapperBuilder()
                .addModule(mod)
                .build();
        Map<?,?> result = mapper.readValue(VPackUtils.toVPack("{}"), Map.class);
    ...
    public void testDefaultingRecursive() throws Exception
    {
        ...
        result = mapper.readValue(VPackUtils.toVPack("[ {} ]"), Object.class);
        assertEquals(LinkedList.class, result.getClass());
        Object v = ((List<?>) result).get(0);
        assertNotNull(v);
        assertEquals(TreeMap.class, v.getClass());

        result = mapper.readValue(VPackUtils.toVPack("{ \"x\": [ 3 ] }"), Object.class);
        assertEquals(TreeMap.class, result.getClass());
        Map<?,?> map = (Map<?,?>) result;
    ...
    public void testInterfaceDefaulting() throws Exception
    {
        ...
        ObjectMapper mapper = vpackMapperBuilder()
                .addModule(mod)
                .build();
        Object result = mapper.readValue(VPackUtils.toVPack(q("abc")), CharSequence.class);
        ...
        mapper = vpackMapperBuilder()
                .addModule(mod)
                .build();
        Abstract a = mapper.readValue(VPackUtils.toVPack("{}"), Abstract.class);
        assertNotNull(a);
    }
    ...
    public void testAbstractMappingsFromTwoModules() throws Exception
    {
        ...
        Datatype1 value1 = mapper.readValue(VPackUtils.toVPack(JSON_EXAMPLE), Datatype1.class);
        assertNotNull(value1);

        Datatype2 value2 = mapper.readValue(VPackUtils.toVPack(JSON_EXAMPLE), Datatype2.class);
