...
//            return r.readValues(new StringReader(json));
        case STRING:
        default:
            return r.readValues(VPackUtils.toVPack(json));
        }
    }
    ...
    public void testRootBeansInArray() throws Exception
    {
        ...
        MappingIterator<Bean> it = MAPPER.readerFor(Bean.class).readValues(VPackUtils.toVPack(JSON));
        ...
        // Also, test 'readAll()'
        it = MAPPER.readerFor(Bean.class).readValues(VPackUtils.toVPack(JSON));
        List<Bean> all = it.readAll();
        assertEquals(2, all.size());
        it.close();

        it = MAPPER.readerFor(Bean.class).readValues(VPackUtils.toVPack("[{\"a\":4},{\"a\":4}]"));
        Set<Bean> set = it.readAll(new HashSet<Bean>());
        assertEquals(HashSet.class, set.getClass());
        assertEquals(1, set.size());
    ...
    public void testHasNextWithEndArrayManagedParser() throws Exception {
        ...
        Iterator<Integer> it = MAPPER.readerFor(Integer.class).readValues(VPackUtils.toVPack(JSON));
        assertTrue(it.hasNext());
        int value = it.next();
        assertEquals(1, value);
        assertTrue(it.hasNext());
    ...
    private void _testObjectReaderWithFastDoubleParser() throws Exception
    {
        ...
        final MappingIterator<Map<String, Double>> iterator = mapper.reader().forType(new TypeReference<Map<String, Double>>(){}).readValues(VPackUtils.toVPack(JSON));
    ...
    private void _testObjectReaderWithFastFloatParser() throws Exception
    {
        ...
        final ObjectMapper mapper = VPackMapper.builder()
                    ...
                    .enable(StreamReadFeature.USE_FAST_BIG_NUMBER_PARSER)
                    .build();
        final MappingIterator<Map<String, Float>> iterator = mapper.reader().forType(new TypeReference<Map<String, Float>>(){}).readValues(VPackUtils.toVPack(JSON));
