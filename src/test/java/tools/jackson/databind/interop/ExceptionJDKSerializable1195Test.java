...
public class ExceptionJDKSerializable1195Test extends DatabindTestUtil
{
    ...
    {
        try {
            MAPPER.readValue(VPackUtils.toVPack("{\"x\": \"B\"}"), ClassToRead.class);
        ...
        try {
            MAPPER.readValue(VPackUtils.toVPack("{\"classToRead\": {\"x\": \"B\"}}"), ContainerClassToRead.class);
            fail("Should not have passed");
        } catch (MismatchedInputException e) {
            verifyException(e, "Cannot deserialize value of type `int` from String \"B\": not a valid `int` value");
            _testSerializability(e);
    ...
    {
        try {
            MAPPER.readValue(VPackUtils.toVPack("{\"classesToRead\": [{\"x\": 1}, {\"x\": \"B\"}]}"),
