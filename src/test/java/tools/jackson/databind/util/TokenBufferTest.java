...
public class TokenBufferTest extends DatabindTestUtil
{
    ...
    void testNumberIntAsStringSerialization() throws IOException {
        try (TokenBuffer buf = new TokenBuffer(null, false)) {
            ...
            assertEquals("42", VPackUtils.toJson(MAPPER.writeValueAsBytes(buf)));
        }
    }
...
//        assertEquals("bah", p.currentName());
    ...
    public void testWithJSONSampleDoc() throws Exception
    {
        ...
        try (JsonParser p = MAPPER.createParser(VPackUtils.toVPack(SAMPLE_DOC_JSON_SPEC))) {
    ...
    private TokenBuffer _tokenBufferFor(Object value) {
        final String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(value));
        TokenBuffer buf = TokenBuffer.forGeneration();
    ...
    public void testBasicSerialize() throws IOException
    {
        ...
        buf = TokenBuffer.forGeneration();
        assertEquals("", VPackUtils.toJson(MAPPER.writeValueAsBytes(buf)));
        buf.close();
        ...
        buf.writeNumber(0.5);
        buf.writeEndArray();
        assertEquals(a2q("[true,false,"+l+",4,0.5]"), VPackUtils.toJson(MAPPER.writeValueAsBytes(buf)));
        buf.close();
    ...
    public void testWithJsonParserSequenceSimple() throws IOException
    {
        ...
        JsonParser p = MAPPER.createParser(VPackUtils.toVPack("[ true, null ]"));
