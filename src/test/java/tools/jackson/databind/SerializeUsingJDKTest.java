...
public class SerializeUsingJDKTest
{
    ...
    public void testEnumHandlers() throws Exception
    {
        ...
        String json = VPackUtils.toJson(mapper.writerFor(EnumPOJO.class)
        ...
        String json2 = VPackUtils.toJson(mapper2.writerFor(EnumPOJO.class)
                .writeValueAsBytes(new EnumPOJO()));
        assertEquals(json, json2);
        EnumPOJO result2 = mapper2.readValue(VPackUtils.toVPack(json2), EnumPOJO.class);
    ...
    public void testObjectMapper() throws Exception
    {
        ...
        assertEquals(EXP_JSON, VPackUtils.toJson(MAPPER.writeValueAsBytes(p)));

        byte[] bytes = jdkSerialize(MAPPER);
        ObjectMapper mapper2 = jdkDeserialize(bytes);
        assertEquals(EXP_JSON, VPackUtils.toJson(mapper2.writeValueAsBytes(p)));
        MyPojo p2 = mapper2.readValue(VPackUtils.toVPack(EXP_JSON), MyPojo.class);
    ...
    public void testObjectWriter() throws Exception
    {
        ...
        final String EXP_JSON = "{\"x\":2,\"y\":3}";
        final MyPojo p = new MyPojo(2, 3);
        assertEquals(EXP_JSON, VPackUtils.toJson(origWriter.writeValueAsBytes(p)));
        String json = VPackUtils.toJson(origWriter.writeValueAsBytes(new AnyBean()
                .addEntry("a", "b")));
    ...
    public void testObjectReader() throws Exception
    {
        ...
        String JSON = "{\"x\":1,\"y\":2}";
        MyPojo p1 = origReader.readValue(VPackUtils.toVPack(JSON));
        assertEquals(2, p1.y);
        ObjectReader anyReader = mapper2.readerFor(AnyBean.class);
        AnyBean any = anyReader.readValue(VPackUtils.toVPack(JSON));
