...
public class ParsingContext2525Test extends DatabindTestUtil
{
    ...
    {
        try (JsonParser p = MAPPER.createParser(VPackUtils.toVPack(MINIMAL_ARRAY_DOC))) {
            _testSimpleArrayUsingPathAsPointer(p);
        }
        try (JsonParser p = MAPPER.createParser(VPackUtils.toVPack(MINIMAL_OBJECT_DOC))) {
            _testSimpleObjectUsingPathAsPointer(p);
        }
        try (JsonParser p = MAPPER.createParser(VPackUtils.toVPack(FULL_DOC))) {
    ...
    {
        try (JsonParser p = MAPPER.createParser(VPackUtils.toVPack(doc))) {
            p.nextToken();
    ...
    {
        JsonNode root = MAPPER.readTree(VPackUtils.toVPack(MINIMAL_ARRAY_DOC));
    ...
    {
        JsonNode root = MAPPER.readTree(VPackUtils.toVPack(MINIMAL_OBJECT_DOC));
        try (JsonParser p = root.traverse(ObjectReadContext.empty())) {
            _testSimpleObjectUsingPathAsPointer(p);
        }
    }
    ...
    {
        JsonNode root = MAPPER.readTree(VPackUtils.toVPack(FULL_DOC));
        try (JsonParser p = root.traverse(ObjectReadContext.empty())) {
            _testFullDocUsingPathAsPointer(p);
        }
    }
