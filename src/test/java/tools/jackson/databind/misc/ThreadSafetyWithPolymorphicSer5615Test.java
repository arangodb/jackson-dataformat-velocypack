    ...
    extends DatabindTestUtil
{
    ...
    public void testConcurrentDeserializationWithJsonIgnoreAndTypeInfo() throws Exception {
        final VPackMapper mapper = newVPackMapper();
        final Result result = mapper.readValue(VPackUtils.toVPack(ROOMS_JSON), Result.class);
            ...
            Thread t = new Thread(() -> {
                try {
                    ...
                    for (int j = 0; j < 100; j++) {
                        ...
                        try {
                            mapper.readValue(VPackUtils.toVPack(json), Result.class);
                        } catch (Throwable e) {
