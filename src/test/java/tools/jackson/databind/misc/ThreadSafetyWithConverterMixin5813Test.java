 ...
 *<p>
 * When multiple threads serialize the same object using the same {@link VPackMapper}, the
    ...
    extends DatabindTestUtil
{
    ...
    private static VPackMapper createMapper() {
    ...
    public void testConcurrentSerializationWithConverterMixin() throws Throwable {
        ...
        VPackMapper mapper = createMapper();
        MyObject myObject = mapper.readValue(VPackUtils.toVPack(expectedJson), MyObject.class);
        ...
        for (int i = 0; i < threadCount; i++) {
            Thread t = new Thread(() -> {
                try {
                    barrier.await();
                    for (int j = 0; j < 100; j++) {
                        String serializedJson = VPackUtils.toJson(mapper.writeValueAsBytes(myObject));
