    ...
    extends DatabindTestUtil
{
    ...
    static class DateAsNumberBean {
        ...
        public Date date;
        public DateAsNumberBean(long l) { date = new Date(l); }
    }

    static class DateAsNumberIntBean {
        @JsonFormat(shape=JsonFormat.Shape.NUMBER_INT)
        public Date date;
        public DateAsNumberIntBean(long l) { date = new Date(l); }
    }

    static class DateAsStringBean {
        @JsonFormat(shape=JsonFormat.Shape.STRING, pattern="yyyy-MM-dd")
        public Date date;
        public DateAsStringBean(long l) { date = new Date(l); }
    }

    static class DateAsDefaultStringBean {
        @JsonFormat(shape=JsonFormat.Shape.STRING)
        public Date date;
        public DateAsDefaultStringBean(long l) { date = new Date(l); }
    }

    static class DateInCETBean {
        @JsonFormat(shape=JsonFormat.Shape.STRING, pattern="yyyy-MM-dd,HH:00", timezone="CET")
        public Date date;
        public DateInCETBean(long l) { date = new Date(l); }
    }
    ...
    static class DateAsDefaultBean {
        public Date date;
        public DateAsDefaultBean(long l) { date = new Date(l); }
    }

    static class DateAsDefaultBeanWithEmptyJsonFormat {
        @JsonFormat
        public Date date;
        public DateAsDefaultBeanWithEmptyJsonFormat(long l) { date = new Date(l); }
    }

    static class DateAsDefaultBeanWithPattern {
        @JsonFormat(pattern="yyyy-MM-dd")
        public Date date;
        public DateAsDefaultBeanWithPattern(long l) { date = new Date(l); }
    }

    static class DateAsDefaultBeanWithLocale {
        @JsonFormat(locale = "fr")
        public Date date;
        public DateAsDefaultBeanWithLocale(long l) { date = new Date(l); }
    }

    static class DateAsDefaultBeanWithTimezone {
        @JsonFormat(timezone="CET")
        public Date date;
        public DateAsDefaultBeanWithTimezone(long l) { date = new Date(l); }
    }
    ...
    private final ObjectMapper WITH_TIMESTAMP_MAPPER = vpackMapperBuilder()
            .enable(DateTimeFeature.WRITE_DATES_AS_TIMESTAMPS)
            .build();

    private final ObjectMapper WITHOUT_TIMESTAMP_MAPPER = vpackMapperBuilder()
    ...
    public void testDateNumeric() throws IOException
    {
        ...
        String json = VPackUtils.toJson(MAPPER.writer().with(DateTimeFeature.WRITE_DATES_AS_TIMESTAMPS)
    ...
    {
        String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(new TimeZoneBean("PST")));
    ...
    public void testDateUsingObjectWriter() throws IOException
    {
        ...
        assertEquals(q("1969-12-31X16:00:00"),
                VPackUtils.toJson(MAPPER.writer(df)
                    .with(tz)
                    .writeValueAsBytes(new Date(0L))));
        ObjectWriter w = MAPPER.writer((DateFormat)null);
        assertEquals("0", VPackUtils.toJson(w.writeValueAsBytes(new Date(0L))));

        w = w.with(df).with(tz);
        assertEquals(q("1969-12-31X16:00:00"), VPackUtils.toJson(w.writeValueAsBytes(new Date(0L))));
    ...
    public void testDatesAsMapKeys() throws IOException
    {
        ...
        assertEquals("{\"1970-01-01T00:00:00.000Z\":1}", VPackUtils.toJson(mapper.writeValueAsBytes(map)));
        ...
        mapper = vpackMapperBuilder()
                ...
                .build();
        assertEquals("{\"0\":1}", VPackUtils.toJson(mapper.writeValueAsBytes(map)));
    }
    ...
    public void testDateWithJsonFormat() throws Exception
    {
        ...
        json = VPackUtils.toJson(MAPPER.writer().without(DateTimeFeature.WRITE_DATES_AS_TIMESTAMPS)
        ...
        // then reverse
        json = VPackUtils.toJson(MAPPER.writer()
                .with(DateTimeFeature.WRITE_DATES_AS_TIMESTAMPS)
                .with(getUTCTimeZone()).writeValueAsBytes(new DateAsStringBean(0L)));
        assertEquals("{\"date\":\"1970-01-01\"}", json);

        // and with different DateFormat; CET is one hour ahead of GMT
        json = VPackUtils.toJson(MAPPER.writeValueAsBytes(new DateInCETBean(0L)));
        assertEquals("{\"date\":\"1970-01-01,01:00\"}", json);

        // and for [Issue#423] as well:
        json = VPackUtils.toJson(MAPPER.writer().with(getUTCTimeZone()).writeValueAsBytes(new CalendarAsStringBean(0L)));
        assertEquals("{\"value\":\"1970-01-01\"}", json);

        // and with default (ISO8601) format (databind#1109)
        json = VPackUtils.toJson(MAPPER.writeValueAsBytes(new DateAsDefaultStringBean(0L)));
    ...
    public void testFormatWithoutPattern() throws Exception
    {
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        String json = VPackUtils.toJson(mapper.writeValueAsBytes(new DateAsDefaultBeanWithTimezone(0L)));
    ...
    public void testDateISO8601_zeroOffsetAsNumeric() throws IOException {
        ...
        // 5) Calendar also respects the feature (exact match, not contains)
        {
            ...
            String json = VPackUtils.toJson(mapper.writeValueAsBytes(cal));
        ...
        // 8) Custom DateFormat is NOT affected by the feature (by design)
        {
            ...
            String json = VPackUtils.toJson(mapper.writeValueAsBytes(date));
        ...
        // 10) WRITE_DATES_AS_TIMESTAMPS ignores the feature (outputs numeric timestamp)
        {
            ObjectMapper mapper = vpackMapperBuilder()
                    ...
                    .enable(DateTimeFeature.WRITE_UTC_AS_OFFSET)
                    .build();
            String json = VPackUtils.toJson(mapper.writeValueAsBytes(new Date(0L)));
            assertEquals("0", json);
        }
        ...
        // Verifies feature applies based on actual offset, not timezone ID
        {
            ...
            String winterJson = VPackUtils.toJson(mapper.writeValueAsBytes(winterDate));
            ...
            String summerJson = VPackUtils.toJson(mapper.writeValueAsBytes(summerDate));
    ...
    private void serialize(ObjectMapper mapper, Object date, String expected) throws IOException {
        assertEquals(q(expected), VPackUtils.toJson(mapper.writeValueAsBytes(date)));
    }

    private void serialize(ObjectMapper mapper, Object date, List<String> expected) throws IOException {
        String result = VPackUtils.toJson(mapper.writeValueAsBytes(date));
    ...
    private void serialize(ObjectWriter w, Object date, String expected) throws IOException {
        assertEquals(q(expected), VPackUtils.toJson(w.writeValueAsBytes(date)));
    }

    private void serialize(ObjectWriter w, Object date, List<String> expected) throws IOException {
        String result = VPackUtils.toJson(w.writeValueAsBytes(date));
        assertTrue(expected.contains(result), "unexpected result: " + result);
    }
