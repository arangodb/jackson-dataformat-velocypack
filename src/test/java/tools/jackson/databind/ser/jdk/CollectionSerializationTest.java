    ...
    extends DatabindTestUtil
{
    ...
    public void testBigCollection() throws IOException
    {
        ...
        for (int mode = 0; mode < 3; ++mode) {
            ...
            case 2:
                {
                    String str = VPackUtils.toJson(MAPPER.writeValueAsBytes(value));
    ...
    public void testListSerializer() throws IOException
    {
        assertEquals(q("[ab, cd, ef]"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new PseudoList("ab", "cd", "ef"))));
        assertEquals(q("[]"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new PseudoList())));
    }
    ...
    public void testEmptyListOrArray() throws IOException
    {
        ...
        assertEquals("{\"empty\":[]}", VPackUtils.toJson(MAPPER.writeValueAsBytes(list)));
        assertEquals("{\"empty\":[]}", VPackUtils.toJson(MAPPER.writeValueAsBytes(array)));
        ...
        ObjectMapper m = vpackMapperBuilder()
                ...
                .build();
        assertEquals("{}", VPackUtils.toJson(m.writeValueAsBytes(list)));
        assertEquals("{}", VPackUtils.toJson(m.writeValueAsBytes(array)));
    }
    ...
    public void testStaticList() throws IOException
    {
        ...
        final ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        json = VPackUtils.toJson(mapper.writeValueAsBytes(w));
    ...
    public void testIterator()
    {
        ...
        assertEquals("[1,null,-9,0]", VPackUtils.toJson(MAPPER.writeValueAsBytes(l.iterator())));
        l.clear();
        assertEquals("[]", VPackUtils.toJson(MAPPER.writeValueAsBytes(l.iterator())));
    }
    ...
    {
        assertEquals("[1,2,3]",
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new IterableWrapper(new int[] { 1, 2, 3 }))));
    }
    ...
    public void testWithIterable()
    {
        assertEquals("{\"values\":[\"value\"]}",
                VPackUtils.toJson(STATIC_MAPPER.writeValueAsBytes(new BeanWithIterable())));
        assertEquals("[1,2,3]",
                VPackUtils.toJson(STATIC_MAPPER.writeValueAsBytes(new IntIterable())));

        assertEquals("{\"values\":[\"value\"]}",
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new BeanWithIterable())));
        assertEquals("[1,2,3]",
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new IntIterable())));

        // 17-Apr-2018, tatu: Turns out we may need "fresh" mapper for some failures?
        ObjectMapper freshMapper = new VPackMapper();
        assertEquals("{\"values\":[\"value\"]}",
                VPackUtils.toJson(freshMapper.writeValueAsBytes(new BeanWithIterable())));
        assertEquals("[1,2,3]",
                VPackUtils.toJson(freshMapper.writeValueAsBytes(new IntIterable())));
    }
    ...
    public void testWithIterator()
    {
        assertEquals("{\"values\":[\"itValue\"]}",
                VPackUtils.toJson(STATIC_MAPPER.writeValueAsBytes(new BeanWithIterator())));

        // [databind#1977]
        ArrayList<Number> numbersList = new ArrayList<>();
        numbersList.add(1);
        numbersList.add(0.25);
        String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(numbersList.iterator()));
    ...
    public void testIterable358() throws Exception {
        String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(new IterB()));
    ...
    {
        assertEquals("[1,2,3]",
                VPackUtils.toJson(STATIC_MAPPER.writeValueAsBytes(new IntIterable2390())));
    }
}
