    ...
    extends DatabindTestUtil
{
    ...
    public void testCloseCloseable() throws Exception
    {
        ...
        ObjectMapper mapper2 = vpackMapperBuilder()
                ...
                .build();
        bean = new CloseableBean();
        VPackUtils.toJson(mapper2.writeValueAsBytes(bean));
        assertTrue(bean.wasClosed);

        // and same via writer
        bean = new CloseableBean();
        VPackUtils.toJson(mapper2.writer()
            .writeValueAsBytes(bean));
        assertTrue(bean.wasClosed);

        // also: let's ensure that ObjectWriter won't interfere with it
        bean = new CloseableBean();
        VPackUtils.toJson(MAPPER.writerFor(CloseableBean.class)
    ...
    public void testCharArrays() throws Exception
    {
        ...
        assertEquals(q("abc"), VPackUtils.toJson(m.writeValueAsBytes(chars)));

        // new feature: serialize as JSON array:
        assertEquals("[\"a\",\"b\",\"c\"]",
                VPackUtils.toJson(m.writer()
    ...
    public void testFlushingAutomatic() throws Exception
    {
        ...
        mapper.writeValue(out, Integer.valueOf(13));
        assertEquals("13", VPackUtils.toJson(out.toByteArray()));
        // ditto with ObjectWriter
        out = new ByteArrayOutputStream();
        ObjectWriter ow = mapper.writer();
        ow.writeValue(out, Integer.valueOf(99));
        assertEquals("99", VPackUtils.toJson(out.toByteArray()));
    }
    ...
    public void testFlushingNotAutomatic() throws Exception
    {
        ...
        // no flushing now:
        assertEquals("", VPackUtils.toJson(out.toByteArray()));
    ...
    public void testSingleElementCollections() throws Exception
    {
        ...
        strs.add("xyz");
        assertEquals(q("xyz"), VPackUtils.toJson(writer.writeValueAsBytes(strs)));
        ArrayList<Integer> ints = new ArrayList<Integer>();
        ints.add(13);
        assertEquals("13", VPackUtils.toJson(writer.writeValueAsBytes(ints)));

        // other Collections, like Sets:
        HashSet<Long> longs = new HashSet<Long>();
        longs.add(42L);
        assertEquals("42", VPackUtils.toJson(writer.writeValueAsBytes(longs)));
        // [databind#180]
        final String EXP_STRINGS = "{\"values\":\"foo\"}";
        assertEquals(EXP_STRINGS, VPackUtils.toJson(writer.writeValueAsBytes(new StringListBean(Collections.singletonList("foo")))));

        final Set<String> SET = new HashSet<String>();
        SET.add("foo");
        assertEquals(EXP_STRINGS, VPackUtils.toJson(writer.writeValueAsBytes(new StringListBean(SET))));

        // arrays:
        assertEquals("true", VPackUtils.toJson(writer.writeValueAsBytes(new boolean[] { true })));
        assertEquals("[true,false]", VPackUtils.toJson(writer.writeValueAsBytes(new boolean[] { true, false })));
        assertEquals("true", VPackUtils.toJson(writer.writeValueAsBytes(new Boolean[] { Boolean.TRUE })));

        assertEquals("3", VPackUtils.toJson(writer.writeValueAsBytes(new short[] { 3 })));
        assertEquals("[3,2]", VPackUtils.toJson(writer.writeValueAsBytes(new short[] { 3, 2 })));

        assertEquals("3", VPackUtils.toJson(writer.writeValueAsBytes(new int[] { 3 })));
        assertEquals("[3,2]", VPackUtils.toJson(writer.writeValueAsBytes(new int[] { 3, 2 })));

        assertEquals("1", VPackUtils.toJson(writer.writeValueAsBytes(new long[] { 1L })));
        assertEquals("[-1,4]", VPackUtils.toJson(writer.writeValueAsBytes(new long[] { -1L, 4L })));

        assertEquals("0.5", VPackUtils.toJson(writer.writeValueAsBytes(new double[] { 0.5 })));
        assertEquals("[0.5,2.5]", VPackUtils.toJson(writer.writeValueAsBytes(new double[] { 0.5, 2.5 })));

        assertEquals("0.5", VPackUtils.toJson(writer.writeValueAsBytes(new float[] { 0.5f })));
        assertEquals("[0.5,2.5]", VPackUtils.toJson(writer.writeValueAsBytes(new float[] { 0.5f, 2.5f })));

        assertEquals(q("foo"), VPackUtils.toJson(writer.writeValueAsBytes(new String[] { "foo" })));
    }
    ...
    public void testEmptyWithAnnotations() throws Exception
    {
        ...
        assertEquals("{}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new EmptyWithAnno())));

        // Including class annotation through mix-ins
        ObjectMapper m2 = vpackMapperBuilder()
                .addMixIn(Empty.class, EmptyWithAnno.class)
                .build();
        assertEquals("{}", VPackUtils.toJson(m2.writeValueAsBytes(new Empty())));
    }
    ...
    {
        // first non-empty:
        assertEquals("{\"value\":123}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new NonZeroWrapper(123))));
        // then empty:
        assertEquals("{}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new NonZeroWrapper(0))));
    }
    ...
    public void testGettersWithoutSetters() throws Exception
    {
        ...
        assertEquals("{\"a\":3,\"b\":4,\"c\":5,\"d\":6}", VPackUtils.toJson(MAPPER.writeValueAsBytes(bean)));

        // but 3 if we require mutator:
        ObjectMapper m = vpackMapperBuilder()
                .enable(MapperFeature.REQUIRE_SETTERS_FOR_GETTERS)
                .build();
        assertEquals("{\"a\":3,\"c\":5,\"d\":6}", VPackUtils.toJson(m.writeValueAsBytes(bean)));
    }
    ...
    public void testGettersWithoutSettersOverride() throws Exception
    {
        ObjectMapper m = vpackMapperBuilder()
                .enable(MapperFeature.REQUIRE_SETTERS_FOR_GETTERS)
                .build();
        assertEquals("{\"a\":123}", VPackUtils.toJson(m.writeValueAsBytes(new GettersWithoutSetters2())));
    }
    ...
    public void testNeedForSetters() throws Exception
    {
        ...
        String json = VPackUtils.toJson(mapper.writeValueAsBytes(dataB));
    ...
    public void testIndentation() throws Exception
    {
        ...
        String result = VPackUtils.toJson(MAPPER.writer().with(SerializationFeature.INDENT_OUTPUT)
    ...
    public void testIndentWithPassedGenerator() throws Exception
    {
        ...
        assertEquals("{\"a\":3}", VPackUtils.toJson(MAPPER.writeValueAsBytes(input)));
        String LF = getLF();
        String INDENTED = "{"+LF+"  \"a\" : 3"+LF+"}";
        final ObjectWriter indentWriter = MAPPER.writer().with(SerializationFeature.INDENT_OUTPUT);
        assertEquals(INDENTED, VPackUtils.toJson(indentWriter.writeValueAsBytes(input)));

        ByteArrayOutputStream out = new ByteArrayOutputStream();
        indentWriter.writeValue(out, input);
        assertEquals(INDENTED, VPackUtils.toJson(out.toByteArray()));
        // and also with ObjectMapper itself
        out = new ByteArrayOutputStream();
        ObjectMapper m2 = vpackMapperBuilder()
                .enable(SerializationFeature.INDENT_OUTPUT)
                .build();
        m2.writeValue(out, input);
        assertEquals(INDENTED, VPackUtils.toJson(out.toByteArray()));
    }
    ...
    public void testNoAccessOverrides() throws Exception
    {
        ObjectMapper m = vpackMapperBuilder()
                ...
                .build();
        assertEquals("{\"x\":1}", VPackUtils.toJson(m.writeValueAsBytes(new SimpleBean())));
    }
