...
public class VPackInclude1757Test extends DatabindTestUtil
{
    static class Entity {
    ...
    public void testGlobalNonDefaultRoundTrip() throws Exception {
        ...
        String json = VPackUtils.toJson(mapper.writeValueAsBytes(entity));
        assertEquals("{}", json);

        // Deserializing {} should restore the default value
        Entity deserialized = mapper.readValue(VPackUtils.toVPack(json), Entity.class);
    ...
    public void testGlobalNonDefaultPrimitiveField() throws Exception {
        ...
        assertEquals("{}", VPackUtils.toJson(mapper.writeValueAsBytes(entity)));

        // Non-default value should be included
        entity.value = 0;
        assertEquals(a2q("{'value':0}"), VPackUtils.toJson(mapper.writeValueAsBytes(entity)));

        // Another non-default value
        entity.value = 99;
        assertEquals(a2q("{'value':99}"), VPackUtils.toJson(mapper.writeValueAsBytes(entity)));
    }
    ...
    public void testGlobalMatchesPerClassBehavior() throws Exception {
        ObjectMapper globalMapper = vpackMapperBuilder()
                .enable(MapperFeature.USE_REAL_INCLUDE_NON_DEFAULT)
                .changeDefaultPropertyInclusion(incl ->
                        incl.withValueInclusion(JsonInclude.Include.NON_DEFAULT))
                .build();

        ObjectMapper defaultMapper = newVPackMapper();

        // Per-class annotated bean
        AnnotatedEntity annotated = new AnnotatedEntity();
        String perClassJson = VPackUtils.toJson(defaultMapper.writeValueAsBytes(annotated));

        // Global NON_DEFAULT bean (same structure, no annotation)
        Entity global = new Entity();
        String globalJson = VPackUtils.toJson(globalMapper.writeValueAsBytes(global));
