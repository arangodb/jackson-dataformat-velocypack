    ...
    extends DatabindTestUtil
{
    ...
    final private ObjectMapper MAPPER = newVPackMapper();
    ...
    {
        assertEquals("{}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new ListBean())));
    }
    ...
    {
        assertEquals("{}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new ArrayBean())));
    }
    ...
    public void testDefaultForIntegers() throws Exception
    {
        ...
        assertEquals("{\"i2\":0}", VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultIntBean(0, Integer.valueOf(0)))));
        assertEquals("{\"i2\":1}", VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultIntBean(0, Integer.valueOf(1)))));
        assertEquals("{}", VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultIntBean(0, null))));
        assertEquals("{\"i1\":3,\"i2\":0}", VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultIntBean(3, Integer.valueOf(0)))));
        assertEquals("{\"i1\":3}", VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultIntBean(3, null))));
        ...
        assertEquals("{}", VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultIntBean(0, Integer.valueOf(0)))));
        assertEquals("{\"i2\":1}", VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultIntBean(0, Integer.valueOf(1)))));
        assertEquals("{}", VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultIntBean(0, null))));
        assertEquals("{\"i1\":3}", VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultIntBean(3, Integer.valueOf(0)))));
        assertEquals("{\"i1\":3}", VPackUtils.toJson(mapper.writeValueAsBytes(new DefaultIntBean(3, null))));
}
    ...
    public void testDefaultForBooleans() throws Exception
    {
        ...
        assertEquals("{\"b2\":false}", VPackUtils.toJson(mapper.writeValueAsBytes(
            new DefaultBooleanBean(false, Boolean.FALSE))));

        // Both true: both included (neither matches their defaults)
        assertEquals("{\"b1\":true,\"b2\":true}", VPackUtils.toJson(mapper.writeValueAsBytes(
        ...
        assertEquals("{}", VPackUtils.toJson(mapper.writeValueAsBytes(
            new DefaultBooleanBean(false, null))));

        // Primitive true, wrapper false: both included (neither matches defaults)
        assertEquals("{\"b1\":true,\"b2\":false}", VPackUtils.toJson(mapper.writeValueAsBytes(
    ...
    public void testIssue5570BooleanWrapper() throws Exception
    {
        ...
        assertEquals("{\"value\":false}", VPackUtils.toJson(mapper.writeValueAsBytes(pojo)));

        // null should be excluded (matches default for Boolean wrapper)
        pojo.setValue(null);
        assertEquals("{}", VPackUtils.toJson(mapper.writeValueAsBytes(pojo)));

        // true should be included (doesn't match null default)
        pojo.setValue(Boolean.TRUE);
        assertEquals("{\"value\":true}", VPackUtils.toJson(mapper.writeValueAsBytes(pojo)));
    }
    ...
    public void testEmptyInclusionScalars() throws Exception
    {
        ...
        ObjectMapper inclMapper = vpackMapperBuilder()
                .changeDefaultPropertyInclusion(incl -> incl.withValueInclusion(JsonInclude.Include.NON_EMPTY))
                .build();

        // First, Strings
        StringWrapper str = new StringWrapper("");
        assertEquals("{\"str\":\"\"}", VPackUtils.toJson(defMapper.writeValueAsBytes(str)));
        assertEquals("{}", VPackUtils.toJson(inclMapper.writeValueAsBytes(str)));
        assertEquals("{}", VPackUtils.toJson(inclMapper.writeValueAsBytes(new StringWrapper())));

        assertEquals("{\"value\":\"x\"}", VPackUtils.toJson(defMapper.writeValueAsBytes(new NonEmptyString("x"))));
        assertEquals("{}", VPackUtils.toJson(defMapper.writeValueAsBytes(new NonEmptyString(""))));

        // Then numbers
        // 11-Nov-2015, tatu: As of Jackson 2.7, scalars should NOT be considered empty,
        //   except for wrappers if they are `null`
        assertEquals("{\"value\":12}", VPackUtils.toJson(defMapper.writeValueAsBytes(new NonEmptyInt(12))));
        assertEquals("{\"value\":0}", VPackUtils.toJson(defMapper.writeValueAsBytes(new NonEmptyInt(0))));

        assertEquals("{\"value\":1.25}", VPackUtils.toJson(defMapper.writeValueAsBytes(new NonEmptyDouble(1.25))));
        assertEquals("{\"value\":0.0}", VPackUtils.toJson(defMapper.writeValueAsBytes(new NonEmptyDouble(0.0))));

        IntWrapper zero = new IntWrapper(0);
        assertEquals("{\"i\":0}", VPackUtils.toJson(defMapper.writeValueAsBytes(zero)));
        assertEquals("{\"i\":0}", VPackUtils.toJson(inclMapper.writeValueAsBytes(zero)));
    }
    ...
    public void test1327ClassDefaultsForEmpty() throws Exception {
        ...
        final String jsonString = VPackUtils.toJson(mapper.writeValueAsBytes(new Issue1327BeanEmpty()));
    ...
    public void test1327ClassDefaultsForAlways() throws Exception {
        ...
        final String jsonString = VPackUtils.toJson(mapper.writeValueAsBytes(new Issue1327BeanAlways()));
    ...
    public void testIssue1351() throws Exception
    {
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        assertEquals(a2q("{}"),
                VPackUtils.toJson(mapper.writeValueAsBytes(new Issue1351Bean(null, (double) 0))));
        // [databind#1417]
        assertEquals(a2q("{}"),
                VPackUtils.toJson(mapper.writeValueAsBytes(new Issue1351NonBean(0))));
    }
    ...
    public void testSerialization4741() throws Exception
    {
        ...
        bean.value = "";
        assertEquals(a2q("{'value':''}"), VPackUtils.toJson(MAPPER.writeValueAsBytes(bean)));
    }
    ...
    public void testInclusionOfDate() throws Exception
    {
        ...
        assertEquals(a2q("{'value':0}"),
                VPackUtils.toJson(writerWith.writeValueAsBytes(new NonEmptyDate(input))));
        assertEquals("{}",
                VPackUtils.toJson(writerWith.writeValueAsBytes(new NonDefaultDate(input))));
    ...
    public void testInclusionOfCalendar() throws Exception
    {
        ...
        assertEquals(a2q("{'value':0}"),
                VPackUtils.toJson(writerWith.writeValueAsBytes(new NonEmptyCalendar(input))));
        assertEquals("{}",
                VPackUtils.toJson(writerWith.writeValueAsBytes(new NonDefaultCalendar(input))));
    }
    ...
    public void testInclusionOfUUID() throws Exception
    {
        ...
        assertEquals(a2q("{'value':'"+nonNullUUID+"'}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new NonEmptyUUID(nonNullUUID))));
        assertEquals("{}",
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new NonEmptyUUID(nullUUID))));

        // and thereby should be considered "Default" value
        assertEquals(a2q("{'value':'"+nonNullUUID+"'}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new NonDefaultUUID(nonNullUUID))));
        assertEquals("{}",
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new NonDefaultUUID(nullUUID))));
    }
}
