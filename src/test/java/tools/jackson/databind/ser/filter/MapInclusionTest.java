...
public class MapInclusionTest extends DatabindTestUtil
{
    ...
    {
        assertEquals(a2q("{}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new NoEmptiesMapContainer().add("a", null).add("b", ""))));
    }
    ...
    public void testNoNullsMap() throws Exception
    {
        assertEquals(a2q("{'stuff':{'b':''}}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new NoNullsMapContainer().add("a", null).add("b", ""))));
    }
    ...
    {
        assertEquals(a2q("{'stuff':{'b':''}}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new NoNullsNotEmptyMapContainer().add("a", null).add("b", ""))));

        assertEquals(a2q("{}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new NoNullsNotEmptyMapContainer().add("a", null).add("b", null))));
    }
    ...
    {
        assertEquals(a2q("{}"), VPackUtils.toJson(MAPPER.writeValueAsBytes(new TopLevel2909())));
    }
    ...
    public void test2572MapDefault() throws Exception
    {
        ...
        assertEquals(a2q("{'Speed':100}"),
                VPackUtils.toJson(mapper.writeValueAsBytes(carProperties)));
        assertEquals(a2q("{'model':'F60','properties':{'Speed':100}}"),
                VPackUtils.toJson(mapper.writeValueAsBytes(car)));
    }
    ...
    public void testNonEmptyViaClass() throws Exception {
        assertEquals(a2q("{'map':{'a':'b'}}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new Bean1649("a", "b"))));
        // null value → map content excluded → empty map → map itself excluded
        assertEquals(a2q("{}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new Bean1649("a", null))));
        // empty string value → excluded by NON_EMPTY
        assertEquals(a2q("{}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new Bean1649("a", ""))));
    }
}
