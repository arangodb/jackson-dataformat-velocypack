...
public class VPackIncludeCustomTest extends DatabindTestUtil
{
    ...
    final private ObjectMapper MAPPER = new VPackMapper();

    @Test
    public void testSimpleCustomFilter() throws Exception
    {
        assertEquals(a2q("{'value':'x'}"), VPackUtils.toJson(MAPPER.writeValueAsBytes(new FooBean("x"))));
        assertEquals("{}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new FooBean("foo"))));
    }
    ...
    public void testCustomFilterWithMap() throws Exception
    {
        ...
        assertEquals(a2q("{'stuff':{'a':'1','c':'2'}}"), VPackUtils.toJson(MAPPER.writeValueAsBytes(input)));
    }
    ...
    public void testRepeatedCalls() throws Exception
    {
        ...
        assertEquals(a2q("{'value':'x'}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new CountingFooBean("x"))));
        assertEquals(1, CountingFooFilter.counter.get());

        assertEquals("{}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new CountingFooBean("foo"))));
        assertEquals(2, CountingFooFilter.counter.get());

        // except filter will be called again for `null`s, as per [databind#3481]
        assertEquals(a2q("{'value':null}"), VPackUtils.toJson(MAPPER.writeValueAsBytes(new CountingFooBean(null))));
    ...
    {
        try {
            String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(new BrokenBean(null)));
