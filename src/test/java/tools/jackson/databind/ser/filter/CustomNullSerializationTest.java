...
*/
    ...
    {
        assertEquals("null", VPackUtils.toJson(MAPPER.writeValueAsBytes(null)));
    }
    ...
    public void testOverriddenDefaultValueNulls() throws Exception
    {
        ObjectMapper m = vpackMapperBuilder()
                ...
                .build();
        assertEquals("\"foobar\"", VPackUtils.toJson(m.writeValueAsBytes(null)));
    }
    ...
    public void testCustomNulls() throws Exception
    {
        ObjectMapper m = vpackMapperBuilder()
                ...
                .build();
        assertEquals("{\"name\":\"foobar\"}", VPackUtils.toJson(m.writeValueAsBytes(new Bean1())));
        assertEquals("{\"type\":null}", VPackUtils.toJson(m.writeValueAsBytes(new Bean2())));
    }
    ...
    public void testCustomNullForTrees() throws Exception
    {
        ...
        assertEquals("{\"a\":null}", VPackUtils.toJson(MAPPER.writeValueAsBytes(root)));
        ...
        ObjectMapper m = vpackMapperBuilder()
                ...
                .addModule(new SimpleModule()
                        .setDefaultNullValueSerializer(new NullAsFoobarSerializer()))
                .build();
        assertEquals("{\"a\":\"foobar\"}", VPackUtils.toJson(m.writeValueAsBytes(root)));
    }

    public void testNullSerializerForProperty() throws Exception
    {
        assertEquals("{\"a\":\"foobar\"}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new BeanWithNullProps())));
    }
}
