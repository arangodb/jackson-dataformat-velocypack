...
public class VPackValueSerializationTest
    extends DatabindTestUtil
{
    ...
    {
        assertEquals("\"abc\"", VPackUtils.toJson(MAPPER.writeValueAsBytes(new ValueClass<String>("abc"))));
        assertEquals("null", VPackUtils.toJson(MAPPER.writeValueAsBytes(new ValueClass<String>(null))));
    }

    @Test
    public void testSimpleFieldJsonValue() throws Exception
    {
        assertEquals("\"abc\"", VPackUtils.toJson(MAPPER.writeValueAsBytes(new FieldValueClass<String>("abc"))));
        assertEquals("null", VPackUtils.toJson(MAPPER.writeValueAsBytes(new FieldValueClass<String>(null))));
    }
    ...
    {
        assertEquals("\"123\"",
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new ToStringValueClass<Integer>(Integer.valueOf(123)))));
    }
    ...
    {
        assertEquals("\"xyz\"",
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new ToStringValueClass2("xyz"))));
    }
    ...
    public void testDisabling() throws Exception
    {
        assertEquals(a2q("{'x':1,'y':2}"),
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new DisabledJsonValue())));
    }
    ...
    public void testValueWithStaticType() throws Exception
    {
        ...
        assertEquals("{\"a\":\"a\",\"b\":\"b\"}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new ValueWrapper())));

        // then static
        ObjectMapper staticMapper = vpackMapperBuilder()
                .configure(MapperFeature.USE_STATIC_TYPING, true)
                .build();
        assertEquals("{\"a\":\"a\"}", VPackUtils.toJson(staticMapper.writeValueAsBytes(new ValueWrapper())));
    }

    @Test
    public void testMapWithJsonValue() throws Exception {
        // First via method
        assertEquals("{\"a\":\"1\"}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new MapBean())));

        // then field
        assertEquals("{\"b\":\"2\"}", VPackUtils.toJson(MAPPER.writeValueAsBytes(new MapFieldBean())));
    }

    @Test
    public void testWithMap() throws Exception {
        assertEquals("42", VPackUtils.toJson(MAPPER.writeValueAsBytes(new MapAsNumber())));
    }

    @Test
    public void testWithList() throws Exception {
        assertEquals("13", VPackUtils.toJson(MAPPER.writeValueAsBytes(new ListAsNumber())));
    }
    ...
    public void testPolymorphicSerdeWithDelegate() throws Exception
    {
	    ...
	    String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(adder));
	    assertEquals("{\"boingo\":\"boopsy\",\"toAdd\":1}", json);
	    assertEquals(2, MAPPER.readValue(VPackUtils.toVPack(json), AdditionInterface.class).add(1));
    }
    ...
    public void testJsonValueWithCustomOverride() throws Exception
    {
        ...
        assertEquals(q("value"), VPackUtils.toJson(MAPPER.writeValueAsBytes(INPUT)));
        ...
        ObjectMapper mapper = vpackMapperBuilder()
                ...
                .build();
        assertEquals("42", VPackUtils.toJson(mapper.writeValueAsBytes(INPUT)));
    }
    ...
    public void testCollectionViaJsonValue() throws Exception
    {
        assertEquals("[{\"impl\":{\"value\":1}}]",
                VPackUtils.toJson(MAPPER.writeValueAsBytes(new Bean1806())));
    }
    ...
    {
        final String json = VPackUtils.toJson(MAPPER.writeValueAsBytes(new A2822("desc",
    ...
    {
        String result = VPackUtils.toJson(MAPPER.writeValueAsBytes(new ClassGetter<String>(null)));
