...
public class HandlerInstantiationTest
{
    ...
    public void testDeserializer() throws Exception
    {
        VPackMapper mapper = VPackMapper.builder()
                ...
                .build();
        MyBean result = mapper.readValue(VPackUtils.toVPack(q("123")), MyBean.class);
    ...
    public void testKeyDeserializer() throws Exception
    {
        VPackMapper mapper = VPackMapper.builder()
                .handlerInstantiator(new MyInstantiator("abc:"))
                .build();
        MyMap map = mapper.readValue(VPackUtils.toVPack("{\"a\":\"b\"}"), MyMap.class);
        // easiest to test by just serializing...
        assertEquals("{\"KEY\":\"b\"}", VPackUtils.toJson(mapper.writeValueAsBytes(map)));
    }
    ...
    public void testSerializer() throws Exception
    {
        VPackMapper mapper = VPackMapper.builder()
                ...
                .build();
        assertEquals(q("xyz:456"), VPackUtils.toJson(mapper.writeValueAsBytes(new MyBean("456"))));
    }
    ...
    public void testTypeIdResolver() throws Exception
    {
        VPackMapper mapper = VPackMapper.builder()
                ...
                .build();
        String json = VPackUtils.toJson(mapper.writeValueAsBytes(new TypeIdBeanWrapper(new TypeIdBean(123))));
        // should now use our custom id scheme:
        assertEquals("{\"bean\":[\"!!!\",{\"x\":123}]}", json);
        // and bring it back too:
        TypeIdBeanWrapper result = mapper.readValue(VPackUtils.toVPack(json), TypeIdBeanWrapper.class);
